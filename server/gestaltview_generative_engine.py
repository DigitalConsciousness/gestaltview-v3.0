...
from server.core.web_search import WebSearchRouter, ground_message
...
class Config:
    ...
    ENABLE_GROQ:        bool = bool(GROQ_API_KEY)
...
class LLMRouter:
    ...
    async def generate(self, request: LLMRequest,
        ...
        start = time.time()
        prompt_parts = []
        if request.system_hint:
            prompt_parts.append(request.system_hint.strip())
        prompt_parts.append(request.prompt.strip())
        ...
        # Try external LLM providers (OpenAI, Groq)
        result = await self._try_external(prompt, request)
        if result:
            return result
    ...
    async def _try_external(self, prompt: str, request: LLMRequest) -> Optional[LLMResponse]:
        """Try external LLM providers: Groq (preferred), then OpenAI."""
        start = time.time()
        logger = logging.getLogger(__name__)
        
        # Try Groq first (higher priority for this deployment)
        if self.config.ENABLE_GROQ:
            try:
                import httpx
                async with httpx.AsyncClient() as client:
                    response = await client.post(
                        ...
                        json={
                            ...
                            "temperature": 0.7,
                            "max_tokens": min(request.max_tokens or 2000, 4096),
                        },
                        timeout=30.0,
                    )
                    ...
                    if response.status_code == 200:
                        ...
                        # Log non-200 responses to aid troubleshooting without overloading logs
                        body_preview = ""
                        try:
                        ...
                        logger.warning(
                            "[v0] Groq API non-200 response: status=%s, body_preview=%s",
                            response.status_code,
                            body_preview,
                        )
            except Exception as e:
                logger.warning(f"[v0] Groq API error: {str(e)}")
                pass
        
        # Fallback to OpenAI if Groq failed or unavailable
        if self.config.ENABLE_OPENAI:
            try:
                ...
                logger.warning(f"[v0] OpenAI API error: {str(e)}")
                pass
...
class BlackboardResponder:
    def __init__(
        ...
        web_search_router: Optional[WebSearchRouter] = None,
    ):
        self.llm     = llm_router
        self.fusion  = fusion_engine
        self.web_search = web_search_router or WebSearchRouter()
    ...
    async def respond(self, request: BlackboardResponderRequest) -> BlackboardResponderResponse:
        ...
        grounding = await ground_message(request.message, self.web_search)
        grounding_block = grounding.to_prompt_block(self.web_search.config.MAX_CONTEXT_CHARS)
        if grounding_block:
        ...
        grounding_summary = {
            "searched": grounding.success,
            "source": grounding.source,
            "query": grounding.query,
            "latency_ms": round(grounding.latency_ms, 2),
            "result_count": len(grounding.results),
        }
        provenance.compute_transformation_hash(
            inputs={"message": request.message, "persona": request.persona_slug, "web_grounding": grounding_summary},
            operations=[
                "persona_routing",
                "web_search" if grounding.success else "web_search_skipped",
                "context_injection",
                "llm_generation",
            ],
        )
        return BlackboardResponderResponse(
            ...
            council_metadata={"web_grounding": grounding_summary},
...
async def sentry_debug():
    logger = logging.getLogger(__name__)
    logger.warning("Sentry debug route triggered")
    1 / 0
