...
| Orchestration execution | Creation Corner orchestration rail + `client/src/lib/orchestratorClient.ts` | Deterministic worker planning and execution, not a general chat LLM path | `/api/orchestrator/decide`, `/api/orchestrator/extract`, `/api/orchestrator/execute` |
...
The orchestration routes are similar in spirit but narrower in scope:

- `/api/orchestrator/decide` returns the deterministic decision plus a worker-plan preview.
- `/api/orchestrator/extract` isolates signal extraction for profile and scaffold-oriented work.
- `/api/orchestrator/execute` runs the worker spine, persists receipts, and applies the presentation gate before returning results to Creation Corner.
