...
## Section 0 — Profile Portrait Landing State

The `profile_portrait` layer from `specs/gen-engine/SPEC-GenEngine-ProfilePortrait-Layer.md` now has a concrete read-model and rendering surface in this repo.

What landed:

- Added a shared portrait model in `shared/profilePortrait.ts` that deterministically synthesizes 10 portrait dimensions from personality-profile output plus live evidence such as memory entries, bucket drops, gravity reports, and founder context.
- Extended `api/profile/personality.ts` to return both the existing personality profile and a portrait read model.
- Extended `api/consciousness/dynamic-inner-world.ts` to include a portrait preview in its response.
- Wired `client/src/components/ProfileDisplay.tsx` so Sanctuary and Dynamic Inner World can render portrait title, tagline, evidence counts, and per-dimension detail.
- Added `profile_portrait` to the Codex artifact contract, template registry, exporter registry, and HTML renderer so the kind now validates and renders deterministically.
- Added the portrait persistence spine in Supabase migrations:
  - `profile_portraits`
  - `portrait_dimensions`
  - `portrait_inference_runs`
  - `portrait_inference_queue`
  - helper RPCs for current version and signal counting
  - a bucket-drop trigger that enqueues threshold-based portrait inference
- Added `api/cron/profile-portrait-drain.ts`, which claims queued portrait jobs, builds the next portrait version, persists the validated portrait and dimensions, and closes out the inference run and queue rows.
- Updated the profile and Dynamic Inner World readers so they prefer the latest validated persisted portrait when one exists.

What is still not implemented:

- The spec’s monthly cadence trigger is still not implemented.
- The queue drain is deterministic and service-role driven, not an LLM-based model worker.
- `profile_portrait` is still not emitted automatically by a forge route; the new kind is contract-supported, renderable, and now backed by persistence tables plus a cron drain, but it is still a manually constructed artifact path outside that cron.
