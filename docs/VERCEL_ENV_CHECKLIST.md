...
If you want a copy/paste subset for the voice rollout, use [docs/VERCEL_VOICE_ENV_TEMPLATE.md](/workspaces/gestaltview-v3.0/docs/VERCEL_VOICE_ENV_TEMPLATE.md).
...
| `DEEPGRAM_API_KEY` | `/api/voice/billy` and the Python voice worker |
| `DEEPGRAM_BILLY_TTS_MODEL` | Billy TTS model selection |
| `DEEPGRAM_BILLY_TTS_SPEED` | Optional playback speed hint |
| `VOICE_PROFILE_SLUG` | Worker voice-profile selection |
...
| `VITE_BILLY_VOICE` | `deepgram` for hosted playback or `browser` for explicit browser fallback |
| `VITE_BILLY_BROWSER_VOICE_NAME` | Optional browser voice preference when browser fallback is enabled |
