...
function buildMindMapDiagram(artifact) {
...
function renderSessionRecap(artifact) {
...
function renderBlueprint(artifact) {
  return buildHtmlShell({
    kind: artifact.kind,
    title: artifact.title,
    subtitle: artifact.body.summary,
...
function renderReportDocument(artifact) {
  return buildHtmlShell({
    kind: artifact.kind,
    title: artifact.title,
    subtitle: artifact.body.summary,
...
function renderMindMap(artifact) {
...
function renderShareCard(artifact) {
  return buildHtmlShell({
    kind: artifact.kind,
    title: artifact.title,
...
function renderCodeModule(artifact) {
  return buildHtmlShell({
    kind: artifact.kind,
    title: artifact.title,
...
function renderSpatialScene(artifact) {
...
function renderAudioNarration(artifact) {
...
function renderProfilePortrait(artifact) {
  ...
  switch (artifact.kind) {
    ...
    default:
      return buildHtmlShell({
        kind: artifact.kind,
        title: artifact.title,
        subtitle: renderArtifactSummary(artifact),
        meta: { artifactId: artifact.id },
        bodyContent: `<section class="gv-section gv-card gv-prose">${renderParagraphs(JSON.stringify(artifact.body, null, 2))}</section>`,
      });
  }
}
