    ...
    case "profile_portrait":
      return {
        html: wrapHtml(
          artifact.title,
          [
            `<h1>${artifact.body.portraitTitle}</h1>`,
            `<p>${artifact.body.tagline}</p>`,
            `<p>${Math.round(artifact.body.overallConfidence * 100)}% confidence</p>`,
            ...artifact.body.dimensions.map(
              (dimension) =>
                `<section><h2>${dimension.label}</h2><p>${dimension.summary}</p></section>`,
            ),
          ].join(""),
        ),
      };
