-- Source: supabase_schema.zip/supabase/migrations/20260327_fix_vector_dims.sql
-- Canonicalized filename: 20260327094600_fix_vector_dims.sql
  ...
  if exists (
    ...
    where table_name = 'skill_fragments'
      ...
      and udt_name = 'vector'
        ...
        join pg_class on pg_class.oid = pg_attribute.attrelid
        where relname = 'skill_fragments' and attname = 'embedding') <> 768 then
    drop view if exists skill_stats;
    ...
    group by skill_name
    order by fragment_count desc, skill_name asc;
  end if;
  ...
  if exists (
    ...
    where table_name = 'embeddings'
      ...
      and udt_name = 'vector'
        ...
        join pg_class on pg_class.oid = pg_attribute.attrelid
        where relname = 'embeddings' and attname = 'embedding') <> 768 then
