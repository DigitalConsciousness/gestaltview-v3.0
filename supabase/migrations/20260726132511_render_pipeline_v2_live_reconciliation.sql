-- Forward-only reconciliation for production environments where
-- 202607130001_render_pipeline_contract_v2.sql is absent from migration history.
-- This remains safe after the historical v2 migration has already run.
...
alter table public.render_jobs
  ...
  add column if not exists request_version text not null
    default 'gestaltview.render-request.v2';

alter table public.render_jobs
  drop constraint if exists render_jobs_targets_array_check,
  drop constraint if exists render_jobs_status_check;

-- The live baseline has no render jobs. On an upgrade fixture, preserve the
-- historical success meaning while adopting the canonical v2 lifecycle.
...
create index if not exists render_artifacts_user_target_created_idx
  on public.render_artifacts (user_id, target_status, created_at desc);
...
-- Ask PostgREST to refresh its schema without changing application data.
