  ...
  if v_allowed = 0 then
    return jsonb_build_object(
      'error', jsonb_build_object(
        'http_code', 403,
        'message',   'GestaltView is currently invite-only.'
      )
    );
  end if;
...
begin
  ...
  -- Pull from app_users (from 001_rate_limits_and_users.sql)
  select
    coalesce(is_admin, false),
    coalesce(is_founder, false),
    coalesce(role::text, 'user'),
    coalesce(plan::text, 'free')
  into v_is_admin, v_is_founder, v_app_role, v_plan
...
CREATE OR REPLACE FUNCTION "public"."fn_corpus_harvest_on_pipeline_complete"() RETURNS "trigger"
    LANGUAGE "plpgsql" SECURITY DEFINER
    SET "search_path" TO 'public', 'pg_temp'
    AS $$
DECLARE
  v_anon_payload jsonb;
BEGIN
  ...
  -- Anonymize: strip subject_id, auth_user_id, preserve structural outputs only
  v_anon_payload := jsonb_build_object(
    'personality_profile',   NEW.output_payload -> 'personality_profile',
    'plk_snapshot',          NEW.output_payload -> 'plk_snapshot',
    'key_themes',            NEW.output_payload -> 'key_themes',
    'unresolved_tensions',   NEW.output_payload -> 'unresolved_tensions',
    'core_narrative',        NEW.output_payload -> 'core_narrative',
    'run_kind',              NEW.run_kind,
    'sector_adaptation',     NEW.input_payload  -> 'sectorAdaptation',
    'harvested_at',          to_jsonb(now())
  );

  INSERT INTO public.corpus_harvest_events (
    source_run_id,
    anonymized_payload,
    harvest_status
  ) VALUES (
...
ALTER FUNCTION "public"."fn_corpus_harvest_on_pipeline_complete"() OWNER TO "postgres";
...
CREATE TABLE IF NOT EXISTS "public"."scaffold_nodes" (
    ...
    "connection_payload" "jsonb" DEFAULT '{}'::"jsonb" NOT NULL,
    ...
    "user_id" "uuid",
    CONSTRAINT "scaffold_nodes_node_kind_check" CHECK (("node_kind" = ANY (ARRAY['candidate'::"text", 'artifact'::"text", 'connection'::"text"]))),
    CONSTRAINT "scaffold_nodes_review_gate" CHECK ((("state" <> ALL (ARRAY['approved'::"text", 'denied'::"text"])) OR (("reviewed_at" IS NOT NULL) AND ("reviewed_by" IS NOT NULL)))),
    CONSTRAINT "scaffold_nodes_state_check" CHECK (("state" = ANY (ARRAY['pending'::"text", 'approved'::"text", 'denied'::"text", 'connected'::"text", 'dormant'::"text", 'released'::"text", 'deleted_by_user'::"text"])))
);
...
CREATE TABLE IF NOT EXISTS "public"."provenance_envelopes" (
    ...
    "ots_status" "text" DEFAULT 'pending'::"text" NOT NULL,
    "ots_receipt_path" "text",
    "bitcoin_attestation" "jsonb",
    "created_at" timestamp with time zone DEFAULT "now"() NOT NULL,
...
CREATE TABLE IF NOT EXISTS "public"."profile_pipeline_runs" (
    ...
    "run_type" "text" DEFAULT 'ingestion'::"text" NOT NULL,
    "error_message" "text",
    "started_at" timestamp with time zone,
    "user_id" "uuid",
    CONSTRAINT "profile_pipeline_runs_initiated_by_check" CHECK (("initiated_by" = ANY (ARRAY['user'::"text", 'billy'::"text", 'system'::"text", 'trainer'::"text", 'migration'::"text"]))),
    CONSTRAINT "profile_pipeline_runs_run_kind_check" CHECK (("run_kind" = ANY (ARRAY['ingestion'::"text", 'capture_backfill'::"text", 'scaffold_review'::"text", 'artifact_synthesis'::"text", 'claim_promotion'::"text", 'embodiment_compile'::"text", 'migration_backfill'::"text"]))),
    CONSTRAINT "profile_pipeline_runs_status_check" CHECK (("status" = ANY (ARRAY['pending'::"text", 'processing'::"text", 'complete'::"text", 'error'::"text", 'cancelled'::"text"])))
);
...
CREATE TABLE IF NOT EXISTS "public"."identity_claims" (
    ...
    "support_capture_ids" "uuid"[] DEFAULT '{}'::"uuid"[] NOT NULL,
    "support_artifact_ids" "uuid"[] DEFAULT '{}'::"uuid"[] NOT NULL,
    "support_scaffold_node_ids" "uuid"[] DEFAULT '{}'::"uuid"[] NOT NULL,
    "confidence" numeric(5,4) DEFAULT 0.50 NOT NULL,
    ...
    "user_id" "uuid",
    CONSTRAINT "identity_claims_claim_type_check" CHECK (("claim_type" = ANY (ARRAY['skill'::"text", 'pattern'::"text", 'personality'::"text", 'value'::"text", 'growth_arc'::"text", 'working_style'::"text"]))),
    CONSTRAINT "identity_claims_confidence_check" CHECK ((("confidence" >= (0)::numeric) AND ("confidence" <= (1)::numeric))),
    CONSTRAINT "identity_claims_proposed_by_check" CHECK (("proposed_by" = ANY (ARRAY['user'::"text", 'billy'::"text", 'system'::"text", 'import'::"text", 'trainer'::"text"]))),
    CONSTRAINT "identity_claims_status_check" CHECK (("status" = ANY (ARRAY['pending'::"text", 'approved'::"text", 'rejected'::"text", 'archived'::"text"]))),
    CONSTRAINT "identity_claims_status_review_gate" CHECK (((("status" <> 'approved'::"text") AND ("status" <> 'rejected'::"text")) OR ("reviewer_user_id" IS NOT NULL)))
);
...
CREATE TABLE IF NOT EXISTS "public"."profile_pipeline_run_links" (
    ...
    "metadata" "jsonb" DEFAULT '{}'::"jsonb" NOT NULL,
    "created_at" timestamp with time zone DEFAULT "now"() NOT NULL,
    "object_type" "text" DEFAULT 'source'::"text" NOT NULL,
    "object_id" "text" DEFAULT ''::"text" NOT NULL,
    CONSTRAINT "profile_pipeline_run_links_link_role_check" CHECK (("link_role" = ANY (ARRAY['input_capture'::"text", 'input_scaffold'::"text", 'input_artifact'::"text", 'output_artifact'::"text", 'output_claim'::"text", 'output_embodiment'::"text", 'legacy_run'::"text"])))
);
...
CREATE TABLE IF NOT EXISTS "public"."capture_events" (
    ...
    "preservation_status" "text" DEFAULT 'active'::"text" NOT NULL,
    "content_hash" "text",
    "display" "jsonb" DEFAULT '{}'::"jsonb" NOT NULL,
    "consent_state" "jsonb" DEFAULT '{}'::"jsonb" NOT NULL,
    "metadata" "jsonb" DEFAULT '{}'::"jsonb" NOT NULL,
    "provenance" "jsonb" DEFAULT '{}'::"jsonb" NOT NULL,
    "source_bucket_drop_id" "uuid",
    "created_at" timestamp with time zone DEFAULT "now"() NOT NULL,
    "updated_at" timestamp with time zone DEFAULT "now"() NOT NULL,
    ...
    "user_id" "uuid",
    CONSTRAINT "capture_events_capture_type_check" CHECK (("capture_type" = ANY (ARRAY['text'::"text", 'voice'::"text", 'audio'::"text", 'image'::"text", 'video'::"text", 'file'::"text", 'code'::"text", 'song'::"text", 'sketch'::"text", 'html'::"text", 'blueprint'::"text"]))),
    CONSTRAINT "capture_events_preservation_status_check" CHECK (("preservation_status" = ANY (ARRAY['active'::"text", 'dormant'::"text", 'released'::"text", 'deleted_by_user'::"text"]))),
    CONSTRAINT "capture_events_source_room_check" CHECK (("source_room" = ANY (ARRAY['sanctuary'::"text", 'blackboard'::"text", 'dynamic_inner_world'::"text", 'creation_corner'::"text", 'external_scaffold'::"text", 'import'::"text"])))
);
...
CREATE OR REPLACE FUNCTION "public"."gv_touch_updated_at"() RETURNS "trigger"
    LANGUAGE "plpgsql"
    SET "search_path" TO 'public', 'pg_temp'
    AS $$
begin
...
ALTER FUNCTION "public"."gv_touch_updated_at"() OWNER TO "postgres";
...
CREATE OR REPLACE FUNCTION "public"."matchSkills"("query_embedding" "extensions"."vector", "match_threshold" double precision DEFAULT 0.7, "match_count" integer DEFAULT 10, "filter_skill_name" "text" DEFAULT NULL::"text", "filter_category" "text" DEFAULT NULL::"text", "filter_tags" "text"[] DEFAULT NULL::"text"[]) RETURNS TABLE("id" "uuid", "skill_name" "text", "skill_category" "text", "content" "text", "source_file" "text", "chunk_index" integer, "total_chunks" integer, "tags" "text"[], "similarity" double precision)
    LANGUAGE "sql"
    SET "search_path" TO 'public'
    AS $$
  SELECT * FROM public.match_skills(
    query_embedding, match_threshold, match_count,
    filter_skill_name, filter_category, filter_tags
  );
$$;


ALTER FUNCTION "public"."matchSkills"("query_embedding" "extensions"."vector", "match_threshold" double precision, "match_count" integer, "filter_skill_name" "text", "filter_category" "text", "filter_tags" "text"[]) OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "public"."match_knowledge_fragments"("query_embedding" "extensions"."vector", "match_count" integer DEFAULT 8, "filter_type" "text" DEFAULT NULL::"text", "filter_package" "text" DEFAULT NULL::"text") RETURNS TABLE("id" "uuid", "content" "text", "source_file" "text", "document_type" "text", "chunk_index" integer, "tags" "text"[], "similarity" double precision)
    LANGUAGE "sql" STABLE
    SET "search_path" TO 'public', 'extensions'
    AS $$
  select
...
ALTER FUNCTION "public"."match_knowledge_fragments"("query_embedding" "extensions"."vector", "match_count" integer, "filter_type" "text", "filter_package" "text") OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "public"."match_memories"("query_embedding" "extensions"."vector", "match_threshold" double precision, "match_count" integer, "user_id_filter" "text") RETURNS TABLE("id" "uuid", "user_id" "text", "title" "text", "content" "text", "similarity" double precision)
    LANGUAGE "plpgsql"
    AS $$
BEGIN
  RETURN QUERY
...
ALTER FUNCTION "public"."match_memories"("query_embedding" "extensions"."vector", "match_threshold" double precision, "match_count" integer, "user_id_filter" "text") OWNER TO "postgres";


COMMENT ON FUNCTION "public"."match_memories"("query_embedding" "extensions"."vector", "match_threshold" double precision, "match_count" integer, "user_id_filter" "text") IS 'pgvector cosine similarity search for memory_entries. Used by MemoryEntryRepository.search_by_embedding.';



CREATE OR REPLACE FUNCTION "public"."match_memory_entries"("query_embedding" "extensions"."vector", "match_count" integer DEFAULT 4, "filter_user_id" "text" DEFAULT NULL::"text", "filter_scope" "text" DEFAULT NULL::"text", "filter_kind" "text" DEFAULT NULL::"text") RETURNS TABLE("id" "uuid", "title" "text", "summary" "text", "content" "text", "kind" "text", "scope" "text", "importance" smallint, "pinned" boolean, "tags" "text"[], "similarity" double precision)
    LANGUAGE "sql" STABLE
    SET "search_path" TO 'public', 'extensions'
    AS $$
  select
...
ALTER FUNCTION "public"."match_memory_entries"("query_embedding" "extensions"."vector", "match_count" integer, "filter_user_id" "text", "filter_scope" "text", "filter_kind" "text") OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "public"."match_skill_fragments"("query_embedding" "extensions"."vector", "match_count" integer DEFAULT 8, "filter_skill" "text" DEFAULT NULL::"text") RETURNS TABLE("id" "uuid", "content" "text", "source_file" "text", "skill_name" "text", "chunk_index" integer, "tags" "text"[], "similarity" double precision)
    LANGUAGE "sql" STABLE
    SET "search_path" TO 'public', 'extensions'
    AS $$
  select
...
ALTER FUNCTION "public"."match_skill_fragments"("query_embedding" "extensions"."vector", "match_count" integer, "filter_skill" "text") OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "public"."match_skill_fragments"("query_embedding" "extensions"."vector", "match_threshold" double precision DEFAULT 0.70, "match_count" integer DEFAULT 8, "filter_skill_name" "text" DEFAULT NULL::"text", "filter_tags" "text"[] DEFAULT NULL::"text"[]) RETURNS TABLE("id" "uuid", "content" "text", "skill_name" "text", "source_file" "text", "document_type" "text", "chunk_index" integer, "total_chunks" integer, "char_count" integer, "tags" "text"[], "similarity" double precision)
...
begin
  ...
  where
    ...
    -- Optional: narrow to a specific named skill
    and (
      filter_skill_name is null
      or sf.skill_name = filter_skill_name
    )
    -- Optional: require at least one matching tag
    and (
      filter_tags is null
...
ALTER FUNCTION "public"."match_skill_fragments"("query_embedding" "extensions"."vector", "match_threshold" double precision, "match_count" integer, "filter_skill_name" "text", "filter_tags" "text"[]) OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "public"."match_skills"("query_embedding" "extensions"."vector", "match_threshold" double precision DEFAULT 0.7, "match_count" integer DEFAULT 10, "filter_skill_name" "text" DEFAULT NULL::"text", "filter_category" "text" DEFAULT NULL::"text", "filter_tags" "text"[] DEFAULT NULL::"text"[]) RETURNS TABLE("id" "uuid", "skill_name" "text", "skill_category" "text", "content" "text", "source_file" "text", "chunk_index" integer, "total_chunks" integer, "tags" "text"[], "similarity" double precision)
    LANGUAGE "plpgsql"
    SET "search_path" TO 'public'
    AS $$
BEGIN
  ...
  WHERE
    (filter_skill_name IS NULL OR s.skill_name = filter_skill_name)
    AND (filter_category   IS NULL OR s.skill_category = filter_category)
...
ALTER FUNCTION "public"."match_skills"("query_embedding" "extensions"."vector", "match_threshold" double precision, "match_count" integer, "filter_skill_name" "text", "filter_category" "text", "filter_tags" "text"[]) OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "public"."matchknowledgefragments"("queryembedding" "extensions"."vector", "matchcount" integer DEFAULT 8, "filtertype" "text" DEFAULT NULL::"text") RETURNS TABLE("content" "text", "sourcefile" "text", "similarity" double precision, "chunkindex" integer, "documenttype" "text", "tags" "text")
    LANGUAGE "sql" STABLE
    AS $$
  SELECT
    m.content,
...
ALTER FUNCTION "public"."matchknowledgefragments"("queryembedding" "extensions"."vector", "matchcount" integer, "filtertype" "text") OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "public"."matchknowledgefragments"("queryembedding" "extensions"."vector", "matchcount" integer DEFAULT 8, "filtertype" "text" DEFAULT NULL::"text", "filterpackage" "text" DEFAULT NULL::"text") RETURNS TABLE("content" "text", "sourcefile" "text", "similarity" double precision, "chunkindex" integer, "documenttype" "text", "tags" "text")
    LANGUAGE "sql" STABLE
    SET "search_path" TO 'public', 'extensions'
    AS $$
  select
...
ALTER FUNCTION "public"."matchknowledgefragments"("queryembedding" "extensions"."vector", "matchcount" integer, "filtertype" "text", "filterpackage" "text") OWNER TO "postgres";
...
CREATE OR REPLACE FUNCTION "public"."promote_drop_to_memory"("p_drop_id" "uuid", "p_auth_user_id" "uuid", "p_kind" "text" DEFAULT 'note'::"text", "p_scope" "text" DEFAULT 'personal'::"text") RETURNS "uuid"
    LANGUAGE "plpgsql" SECURITY DEFINER
    SET "search_path" TO 'public', 'pg_temp'
    AS $$
DECLARE
  v_drop          public.bucket_drops%ROWTYPE;
  v_memory_id     uuid;
  v_importance    smallint;
  v_content_hash  text;
BEGIN
  -- ── Guard: fetch and lock the drop row ──────────────────────
  SELECT * INTO v_drop
  FROM   public.bucket_drops
  WHERE  id = p_drop_id
  FOR UPDATE SKIP LOCKED;
  ...
  IF NOT FOUND THEN
    RAISE EXCEPTION 'bucket_drop % not found or locked by another process', p_drop_id;
  END IF;

  IF v_drop.stage = 'promoted' THEN
    -- Idempotent: return existing promoted memory
    RETURN v_drop.promoted_memory_id;
  END IF;

  -- ── Map intensity → memory importance (1–5 scale) ───────────
  -- intensity 1–2 → 1, 3–4 → 2, 5–6 → 3, 7–8 → 4, 9–10 → 5
  v_importance := LEAST(5, CEIL(v_drop.intensity::numeric / 2.0)::smallint);

  -- ── Stable content hash ─────────────────────────────────────
  v_content_hash := encode(sha256(v_drop.content::bytea), 'hex');
  ...
  VALUES (
    ...
    p_scope,
    p_kind,
    LEFT(v_drop.content, 120),                          -- title: first 120 chars
    v_drop.content,                                     -- summary: full drop text
    v_drop.content,                                     -- content: same source
    v_content_hash,
    'bucket_drop',                                      -- source label
    'bucket_drop',                                      -- source_kind enum value
    v_drop.id::text,                                    -- source_ref = drop UUID
    v_drop.tags,
    v_importance,
    NULL,                                               -- emotional_valence: scorer can set later
    true,
    'active',
    jsonb_build_object(
      'bucket_drop_id',      v_drop.id,
      'module_key',          v_drop.module_key,
      'intensity',           v_drop.intensity,
      'plk_resonance_score', v_drop.plk_resonance_score,
      'specialized_apps',    v_drop.specialized_apps,
      'capture_context',     v_drop.capture_context,
      'dropped_at',          v_drop.created_at
    ),
    jsonb_build_object(
      'promoted_by', 'promote_drop_to_memory',
      'promoted_at', now()
    )
  )
  RETURNING id INTO v_memory_id;

  -- ── Back-reference and stage update on the drop ──────────────
  UPDATE public.bucket_drops
  SET
    stage              = 'promoted',
    promoted_memory_id = v_memory_id,
    promoted_at        = now()
  WHERE id = p_drop_id;
...
ALTER FUNCTION "public"."promote_drop_to_memory"("p_drop_id" "uuid", "p_auth_user_id" "uuid", "p_kind" "text", "p_scope" "text") OWNER TO "postgres";


COMMENT ON FUNCTION "public"."promote_drop_to_memory"("p_drop_id" "uuid", "p_auth_user_id" "uuid", "p_kind" "text", "p_scope" "text") IS 'Lifts a bucket_drop into memory_entries. Maps intensity→importance, sets source_kind=''bucket_drop'', carries PLK resonance + capture_context in provenance. Idempotent: re-running returns existing memory id.';
...
CREATE OR REPLACE FUNCTION "public"."searchSkills"("query_text" "text", "match_count" integer DEFAULT 10, "filter_skill_name" "text" DEFAULT NULL::"text", "filter_category" "text" DEFAULT NULL::"text", "filter_tags" "text"[] DEFAULT NULL::"text"[]) RETURNS TABLE("id" "uuid", "skill_name" "text", "skill_category" "text", "content" "text", "source_file" "text", "chunk_index" integer, "total_chunks" integer, "tags" "text"[], "rank" double precision)
    LANGUAGE "sql"
    SET "search_path" TO 'public'
    AS $$
  SELECT * FROM public.search_skills(
...
ALTER FUNCTION "public"."searchSkills"("query_text" "text", "match_count" integer, "filter_skill_name" "text", "filter_category" "text", "filter_tags" "text"[]) OWNER TO "postgres";
...
CREATE OR REPLACE FUNCTION "public"."search_skill_fragments"("query_text" "text", "match_count" integer DEFAULT 8, "filter_skill" "text" DEFAULT NULL::"text") RETURNS TABLE("id" "uuid", "content" "text", "source_file" "text", "skill_name" "text", "chunk_index" integer, "tags" "text"[], "rank" double precision)
    LANGUAGE "sql" STABLE
    SET "search_path" TO 'public', 'extensions'
    AS $$
  select
...
ALTER FUNCTION "public"."search_skill_fragments"("query_text" "text", "match_count" integer, "filter_skill" "text") OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "public"."search_skill_fragments"("search_query" "text", "match_count" integer DEFAULT 8, "filter_skill_name" "text" DEFAULT NULL::"text", "filter_tags" "text"[] DEFAULT NULL::"text"[]) RETURNS TABLE("id" "uuid", "content" "text", "skill_name" "text", "source_file" "text", "document_type" "text", "chunk_index" integer, "total_chunks" integer, "char_count" integer, "tags" "text"[], "rank" double precision)
    LANGUAGE "plpgsql" STABLE PARALLEL SAFE
    SET "search_path" TO 'public', 'extensions'
    AS $$
begin
...
ALTER FUNCTION "public"."search_skill_fragments"("search_query" "text", "match_count" integer, "filter_skill_name" "text", "filter_tags" "text"[]) OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "public"."search_skills"("query_text" "text", "match_count" integer DEFAULT 10, "filter_skill_name" "text" DEFAULT NULL::"text", "filter_category" "text" DEFAULT NULL::"text", "filter_tags" "text"[] DEFAULT NULL::"text"[]) RETURNS TABLE("id" "uuid", "skill_name" "text", "skill_category" "text", "content" "text", "source_file" "text", "chunk_index" integer, "total_chunks" integer, "tags" "text"[], "rank" double precision)
    LANGUAGE "plpgsql"
    SET "search_path" TO 'public'
    AS $$
BEGIN
  ...
  FROM public.skills s
  WHERE
    to_tsvector('english', s.content) @@ plainto_tsquery('english', query_text)
    AND (filter_skill_name IS NULL OR s.skill_name = filter_skill_name)
    AND (filter_category   IS NULL OR s.skill_category = filter_category)
    AND (filter_tags        IS NULL OR s.tags && filter_tags)
  ORDER BY rank DESC
  LIMIT match_count;
...
ALTER FUNCTION "public"."search_skills"("query_text" "text", "match_count" integer, "filter_skill_name" "text", "filter_category" "text", "filter_tags" "text"[]) OWNER TO "postgres";
...
CREATE OR REPLACE FUNCTION "public"."trainer_search_study_sources"("query_text" "text", "query_embedding" "extensions"."vector", "match_threshold" double precision DEFAULT 0.5, "match_limit" integer DEFAULT 20) RETURNS TABLE("fragment_id" "uuid", "content" "text", "source_file" "text", "document_type" "text", "similarity" double precision)
    LANGUAGE "plpgsql"
    AS $$
BEGIN
...
ALTER FUNCTION "public"."trainer_search_study_sources"("query_text" "text", "query_embedding" "extensions"."vector", "match_threshold" double precision, "match_limit" integer) OWNER TO "postgres";
...
CREATE FOREIGN DATA WRAPPER "docs_upload_fdw" HANDLER "extensions"."s3_vectors_fdw_handler" VALIDATOR "extensions"."s3_vectors_fdw_validator";




CREATE SERVER "docs_upload_fdw_server" FOREIGN DATA WRAPPER "docs_upload_fdw" OPTIONS (
    "aws_region" 'us-west-2',
    "endpoint_url" 'https://ltajayfzlaevchxngkrm.storage.supabase.co/storage/v1/vector',
    "vault_access_key_id" '36ac801f-2884-46cf-bdfd-0667534a797a',
    "vault_secret_access_key" 'a0a8286f-ca6b-41b8-b31c-04ffb3bd2f56'
);


ALTER SERVER "docs_upload_fdw_server" OWNER TO "postgres";
...
CREATE TABLE IF NOT EXISTS "public"."_deprecated_artifacts" (
    ...
    "byte_size" bigint,
    CONSTRAINT "artifacts_content_format_check" CHECK (("content_format" = ANY (ARRAY['markdown'::"text", 'html'::"text", 'json'::"text", 'text'::"text", 'code'::"text", 'binary'::"text"]))),
    CONSTRAINT "artifacts_origin_room_check" CHECK (("origin_room" = ANY (ARRAY['blackboard'::"text", 'creation_corner'::"text", 'dynamic_inner_world'::"text", 'external_scaffold'::"text", 'unknown'::"text"]))),
    CONSTRAINT "artifacts_source_kind_check" CHECK (("source_kind" = ANY (ARRAY['capture'::"text", 'scaffold'::"text", 'creation_corner'::"text", 'dynamic_inner_world'::"text", 'import'::"text", 'system'::"text"]))),
    CONSTRAINT "artifacts_status_check" CHECK (("status" = ANY (ARRAY['active'::"text", 'dormant'::"text", 'released'::"text", 'archived'::"text", 'deleted_by_user'::"text"])))
);
...
CREATE TABLE IF NOT EXISTS "public"."_deprecated_build_jobs" (
    "id" "uuid" DEFAULT "gen_random_uuid"() NOT NULL,
    "order_id" "uuid" NOT NULL,
    "package_draft_id" "uuid" NOT NULL,
    "build_version" integer DEFAULT 1 NOT NULL,
    ...
    CONSTRAINT "build_jobs_status_check" CHECK (("status" = ANY (ARRAY['queued'::"text", 'running'::"text", 'packaged'::"text", 'delivered'::"text", 'failed'::"text"])))
);


ALTER TABLE "public"."_deprecated_build_jobs" OWNER TO "postgres";


CREATE TABLE IF NOT EXISTS "public"."_deprecated_buyers" (
    "id" "uuid" DEFAULT "gen_random_uuid"() NOT NULL,
    "email" "text" NOT NULL,
    "company_name" "text",
    "created_at" timestamp with time zone DEFAULT "now"() NOT NULL
...
ALTER TABLE "public"."_deprecated_buyers" OWNER TO "postgres";


CREATE TABLE IF NOT EXISTS "public"."_deprecated_order_items" (
    "id" "uuid" DEFAULT "gen_random_uuid"() NOT NULL,
    "order_id" "uuid" NOT NULL,
...
ALTER TABLE "public"."_deprecated_order_items" OWNER TO "postgres";
...
CREATE TABLE IF NOT EXISTS "public"."_deprecated_package_drafts" (
    "id" "uuid" DEFAULT "gen_random_uuid"() NOT NULL,
    "buyer_id" "uuid",
    ...
    "custom_notes" "text",
    "wants_native_installer" boolean DEFAULT false NOT NULL,
    "price_snapshot_cents" integer DEFAULT 0 NOT NULL,
    ...
    "buyer_email" "text",
    "company_name" "text",
    CONSTRAINT "package_drafts_backend_check" CHECK (("backend" = ANY (ARRAY['supabase'::"text", 'redis'::"text", 'mongodb'::"text"]))),
    CONSTRAINT "package_drafts_seats_requested_check" CHECK (("seats_requested" > 0)),
    CONSTRAINT "package_drafts_status_check" CHECK (("status" = ANY (ARRAY['draft'::"text", 'saved'::"text", 'awaiting_payment'::"text", 'paid'::"text", 'provisioning'::"text", 'packaged'::"text", 'delivered'::"text", 'failed'::"text", 'review_requested'::"text"]))),
    CONSTRAINT "package_drafts_tier_check" CHECK (("tier" = ANY (ARRAY['SOLO_SPARK'::"text", 'STUDIO'::"text", 'GROWTH'::"text", 'ENTERPRISE'::"text"])))
);


ALTER TABLE "public"."_deprecated_package_drafts" OWNER TO "postgres";


CREATE TABLE IF NOT EXISTS "public"."_deprecated_support_requests" (
    "id" "uuid" DEFAULT "gen_random_uuid"() NOT NULL,
    "package_draft_id" "uuid",
    "order_id" "uuid",
...
ALTER TABLE "public"."_deprecated_support_requests" OWNER TO "postgres";
...
COMMENT ON COLUMN "public"."bucket_drops"."subject_id" IS 'Links drop to identity_subjects for downstream memory + context injection';



COMMENT ON COLUMN "public"."bucket_drops"."module_key" IS 'Which GestaltView module owns this drop (e.g. lightning_capture, plk_core)';



COMMENT ON COLUMN "public"."bucket_drops"."intensity" IS 'RPE-style 1–10 signal strength assigned during scoring';



COMMENT ON COLUMN "public"."bucket_drops"."plk_resonance_score" IS 'Normalized 0–1 PLK resonance score from scorer agent';



COMMENT ON COLUMN "public"."bucket_drops"."specialized_apps" IS 'e.g. [''ADHD_POWER_UP'', ''CORE_GESTALTVIEW''] — mirrors notebook enum';



COMMENT ON COLUMN "public"."bucket_drops"."tags" IS 'Free-form tags surfaced during scoring or manual review';



COMMENT ON COLUMN "public"."bucket_drops"."stage" IS 'Lifecycle: raw → scored → promoted → archived';



COMMENT ON COLUMN "public"."bucket_drops"."promoted_memory_id" IS 'FK back-reference to memory_entries row after promotion';



COMMENT ON COLUMN "public"."bucket_drops"."scored_at" IS 'Timestamp when scorer agent ran';



COMMENT ON COLUMN "public"."bucket_drops"."promoted_at" IS 'Timestamp when drop was lifted into memory_entries';
...
CREATE TABLE IF NOT EXISTS "public"."embodiment_profiles" (
    ...
    "created_at" timestamp with time zone DEFAULT "now"() NOT NULL,
    "updated_at" timestamp with time zone DEFAULT "now"() NOT NULL,
    "embodiment_version" "text",
    "source_path" "text",
    "source_checksum" "text",
    "governed_identity" "jsonb" DEFAULT '{}'::"jsonb" NOT NULL,
    ...
    CONSTRAINT "embodiment_profiles_review_state_check" CHECK (("review_state" = ANY (ARRAY['draft'::"text", 'pending_review'::"text", 'approved'::"text", 'rejected'::"text", 'archived'::"text"]))),
...
CREATE TABLE IF NOT EXISTS "public"."gate_artifacts" (
    ...
    "byte_size" bigint,
    "created_at" timestamp with time zone DEFAULT "now"() NOT NULL,
    "download_token" "text" DEFAULT ''::"text" NOT NULL,
    CONSTRAINT "gate_artifacts_byte_size_check" CHECK ((("byte_size" IS NULL) OR ("byte_size" >= 0)))
);


ALTER TABLE "public"."gate_artifacts" OWNER TO "postgres";
...
CREATE TABLE IF NOT EXISTS "public"."gate_build_jobs" (
    ...
    "retry_count" integer DEFAULT 0 NOT NULL,
    "build_log" "jsonb" DEFAULT '[]'::"jsonb" NOT NULL,
    CONSTRAINT "gate_build_jobs_status_check" CHECK (("status" = ANY (ARRAY['queued'::"text", 'running'::"text", 'packaged'::"text", 'delivered'::"text", 'failed'::"text"])))
);


ALTER TABLE "public"."gate_build_jobs" OWNER TO "postgres";
...
ALTER TABLE "public"."gate_buyers" OWNER TO "postgres";
...
CREATE TABLE IF NOT EXISTS "public"."gate_order_items" (
    ...
    "unit_price_cents" integer NOT NULL,
    "metadata" "jsonb" DEFAULT '{}'::"jsonb" NOT NULL,
    CONSTRAINT "gate_order_items_quantity_check" CHECK (("quantity" > 0))
);


ALTER TABLE "public"."gate_order_items" OWNER TO "postgres";
...
CREATE TABLE IF NOT EXISTS "public"."gate_orders" (
    ...
    "created_at" timestamp with time zone DEFAULT "now"() NOT NULL,
    "updated_at" timestamp with time zone DEFAULT "now"() NOT NULL,
    CONSTRAINT "gate_orders_order_status_check" CHECK (("order_status" = ANY (ARRAY['draft'::"text", 'awaiting_payment'::"text", 'paid'::"text", 'provisioning'::"text", 'packaged'::"text", 'delivered'::"text", 'failed'::"text", 'review_requested'::"text"]))),
    CONSTRAINT "gate_orders_payment_status_check" CHECK (("payment_status" = ANY (ARRAY['draft'::"text", 'awaiting_payment'::"text", 'paid'::"text", 'review_requested'::"text", 'failed'::"text"]))),
    CONSTRAINT "gate_orders_subtotal_cents_check" CHECK (("subtotal_cents" >= 0)),
    CONSTRAINT "gate_orders_total_cents_check" CHECK (("total_cents" >= 0))
);


ALTER TABLE "public"."gate_orders" OWNER TO "postgres";
...
CREATE TABLE IF NOT EXISTS "public"."gate_package_drafts" (
    ...
    "buyer_context" "jsonb" DEFAULT '{"preferredChannels": [], "requestedOutcomes": []}'::"jsonb" NOT NULL,
    "sidekick_state" "jsonb" DEFAULT '{}'::"jsonb" NOT NULL,
    CONSTRAINT "gate_package_drafts_backend_check" CHECK (("backend" = ANY (ARRAY['supabase'::"text", 'redis'::"text", 'mongodb'::"text"]))),
    CONSTRAINT "gate_package_drafts_seats_requested_check" CHECK (("seats_requested" > 0)),
    CONSTRAINT "gate_package_drafts_status_check" CHECK (("status" = ANY (ARRAY['draft'::"text", 'saved'::"text", 'awaiting_payment'::"text", 'paid'::"text", 'provisioning'::"text", 'packaged'::"text", 'delivered'::"text", 'failed'::"text", 'review_requested'::"text"]))),
    CONSTRAINT "gate_package_drafts_tier_check" CHECK (("tier" = ANY (ARRAY['SOLO_SPARK'::"text", 'STUDIO'::"text", 'GROWTH'::"text", 'ENTERPRISE'::"text"])))
);


ALTER TABLE "public"."gate_package_drafts" OWNER TO "postgres";
...
ALTER TABLE "public"."gate_support_requests" OWNER TO "postgres";
...
CREATE TABLE IF NOT EXISTS "public"."human_cognition_profiles" (
    ...
    "created_at" timestamp with time zone DEFAULT "now"() NOT NULL,
    "updated_at" timestamp with time zone DEFAULT "now"() NOT NULL,
    CONSTRAINT "human_cognition_profiles_confidence_check" CHECK ((("confidence" >= (0)::numeric) AND ("confidence" <= (1)::numeric)))
);
...
CREATE TABLE IF NOT EXISTS "public"."human_consciousness_profiles" (
    ...
    "created_at" timestamp with time zone DEFAULT "now"() NOT NULL,
    "updated_at" timestamp with time zone DEFAULT "now"() NOT NULL,
    CONSTRAINT "human_consciousness_profiles_confidence_check" CHECK ((("confidence" >= (0)::numeric) AND ("confidence" <= (1)::numeric)))
);
...
CREATE TABLE IF NOT EXISTS "public"."human_context_views" (
    ...
    "created_at" timestamp with time zone DEFAULT "now"() NOT NULL,
    "updated_at" timestamp with time zone DEFAULT "now"() NOT NULL,
    CONSTRAINT "human_context_views_scope_check" CHECK (("scope" = ANY (ARRAY['self'::"text", 'relationship'::"text", 'channel'::"text", 'workspace'::"text"])))
);
...
CREATE TABLE IF NOT EXISTS "public"."human_continuity_snapshots" (
    ...
    "created_at" timestamp with time zone DEFAULT "now"() NOT NULL,
    "updated_at" timestamp with time zone DEFAULT "now"() NOT NULL,
    CONSTRAINT "human_continuity_snapshots_confidence_check" CHECK ((("confidence" >= (0)::numeric) AND ("confidence" <= (1)::numeric))),
    CONSTRAINT "human_continuity_snapshots_snapshot_kind_check" CHECK (("snapshot_kind" = ANY (ARRAY['identity'::"text", 'cognition'::"text", 'consciousness'::"text", 'personality'::"text", 'memory'::"text", 'relationship'::"text", 'context'::"text", 'reflection'::"text", 'handoff'::"text", 'review'::"text"])))
);
...
CREATE TABLE IF NOT EXISTS "public"."human_identity_evidence" (
    ...
    "metadata" "jsonb" DEFAULT '{}'::"jsonb" NOT NULL,
    "created_at" timestamp with time zone DEFAULT "now"() NOT NULL,
    CONSTRAINT "human_identity_evidence_source_type_check" CHECK (("source_type" = ANY (ARRAY['conversation'::"text", 'task'::"text", 'reflection'::"text", 'import'::"text", 'human-review'::"text", 'agent-observation'::"text", 'system-derived'::"text"]))),
    CONSTRAINT "human_identity_evidence_weight_check" CHECK ((("weight" >= (0)::numeric) AND ("weight" <= (1)::numeric)))
);
...
CREATE TABLE IF NOT EXISTS "public"."human_identity_mutations" (
    ...
    "provenance" "jsonb" DEFAULT '{}'::"jsonb" NOT NULL,
    "created_at" timestamp with time zone DEFAULT "now"() NOT NULL,
    CONSTRAINT "human_identity_mutations_confidence_check" CHECK ((("confidence" >= (0)::numeric) AND ("confidence" <= (1)::numeric))),
    CONSTRAINT "human_identity_mutations_evidence_count_check" CHECK (("evidence_count" >= 0))
);
...
CREATE TABLE IF NOT EXISTS "public"."human_identity_profiles" (
    ...
    "created_at" timestamp with time zone DEFAULT "now"() NOT NULL,
    "updated_at" timestamp with time zone DEFAULT "now"() NOT NULL,
    CONSTRAINT "human_identity_profiles_confidence_check" CHECK ((("confidence" >= (0)::numeric) AND ("confidence" <= (1)::numeric)))
);
...
CREATE TABLE IF NOT EXISTS "public"."human_memory_records" (
    ...
    "created_at" timestamp with time zone DEFAULT "now"() NOT NULL,
    "updated_at" timestamp with time zone DEFAULT "now"() NOT NULL,
    CONSTRAINT "human_memory_records_confidence_check" CHECK ((("confidence" >= (0)::numeric) AND ("confidence" <= (1)::numeric))),
    CONSTRAINT "human_memory_records_emotional_valence_check" CHECK ((("emotional_valence" >= ('-1'::integer)::numeric) AND ("emotional_valence" <= (1)::numeric))),
    CONSTRAINT "human_memory_records_evidence_count_check" CHECK (("evidence_count" >= 0)),
    CONSTRAINT "human_memory_records_memory_kind_check" CHECK (("memory_kind" = ANY (ARRAY['identity'::"text", 'preference'::"text", 'goal'::"text", 'project'::"text", 'relationship'::"text", 'constraint'::"text", 'insight'::"text", 'note'::"text", 'reflection'::"text", 'episode'::"text", 'value'::"text", 'boundary'::"text"]))),
    CONSTRAINT "human_memory_records_salience_check" CHECK ((("salience" >= (0)::numeric) AND ("salience" <= (1)::numeric))),
    CONSTRAINT "human_memory_records_scope_check" CHECK (("scope" = ANY (ARRAY['personal'::"text", 'session'::"text", 'shared'::"text", 'reviewed'::"text"])))
);
...
CREATE TABLE IF NOT EXISTS "public"."human_personality_profiles" (
    ...
    "created_at" timestamp with time zone DEFAULT "now"() NOT NULL,
    "updated_at" timestamp with time zone DEFAULT "now"() NOT NULL,
    CONSTRAINT "human_personality_profiles_confidence_check" CHECK ((("confidence" >= (0)::numeric) AND ("confidence" <= (1)::numeric)))
);
...
CREATE TABLE IF NOT EXISTS "public"."human_relationship_edges" (
    ...
    "created_at" timestamp with time zone DEFAULT "now"() NOT NULL,
    "updated_at" timestamp with time zone DEFAULT "now"() NOT NULL,
    CONSTRAINT "human_relationship_edges_confidence_check" CHECK ((("confidence" >= (0)::numeric) AND ("confidence" <= (1)::numeric))),
    CONSTRAINT "human_relationship_edges_distinct_subjects" CHECK (("subject_id" <> "related_subject_id")),
    CONSTRAINT "human_relationship_edges_familiarity_level_check" CHECK ((("familiarity_level" >= (0)::numeric) AND ("familiarity_level" <= (1)::numeric))),
    CONSTRAINT "human_relationship_edges_relationship_type_check" CHECK (("relationship_type" = ANY (ARRAY['self'::"text", 'partner'::"text", 'family'::"text", 'friend'::"text", 'coworker'::"text", 'mentor'::"text", 'mentee'::"text", 'client'::"text", 'collaborator'::"text", 'other'::"text"]))),
    CONSTRAINT "human_relationship_edges_trust_level_check" CHECK ((("trust_level" >= (0)::numeric) AND ("trust_level" <= (1)::numeric)))
);
...
CREATE TABLE IF NOT EXISTS "public"."identity_subjects" (
    ...
    "created_at" timestamp with time zone DEFAULT "now"() NOT NULL,
    "updated_at" timestamp with time zone DEFAULT "now"() NOT NULL,
    CONSTRAINT "identity_subjects_status_check" CHECK (("status" = ANY (ARRAY['active'::"text", 'pending'::"text", 'paused'::"text", 'archived'::"text"])))
);
...
CREATE TABLE IF NOT EXISTS "public"."inner_world_artifacts" (
    ...
    CONSTRAINT "inner_world_artifacts_status_check" CHECK (("status" = ANY (ARRAY['active'::"text", 'archived'::"text"])))
);
...
COMMENT ON COLUMN "public"."knowledge_fragments"."ingested_at" IS 'Timestamp when this fragment was written by the ingest pipeline. Set explicitly by ingest_corpus.py.';



COMMENT ON COLUMN "public"."knowledge_fragments"."file_last_modified" IS 'OS mtime of the source file at the time of ingestion. Distinct from source_created_at (curated canonical date) and ingested_at (pipeline write time).';



COMMENT ON COLUMN "public"."knowledge_fragments"."package" IS 'corpus_map.json package key this fragment belongs to (e.g. corpus, diligence, billy_voice).';



COMMENT ON COLUMN "public"."knowledge_fragments"."run_id" IS 'FK to processing_runs.run_id — links each fragment to the specific pipeline run that produced it.';
...
CREATE TABLE IF NOT EXISTS "public"."kv_store_441770b7" (
    "key" "text" NOT NULL,
    "value" "jsonb" NOT NULL
);


ALTER TABLE "public"."kv_store_441770b7" OWNER TO "postgres";
...
COMMENT ON TABLE "public"."loom_annotations" IS 'LoomAnalyzer output — gaps, threads, motifs, weak connections, and emergent patterns across the corpus';
...
CREATE TABLE IF NOT EXISTS "public"."model_homes" (
    ...
    "model_id" "uuid",
    "ring" "text" NOT NULL,
    "privacy_tier" "text" DEFAULT 'federated'::"text" NOT NULL,
    "modalities" "text"[] DEFAULT '{text}'::"text"[] NOT NULL,
    "tribunal_role" "text",
    "plk_feedback_enabled" boolean DEFAULT false NOT NULL,
    "resonance_rescore_enabled" boolean DEFAULT true NOT NULL,
    "corpus_harvest_enabled" boolean DEFAULT false NOT NULL,
    "consent_profile" "jsonb" DEFAULT '{}'::"jsonb" NOT NULL,
    "last_loom_pass" timestamp with time zone,
    "status" "text" DEFAULT 'active'::"text" NOT NULL,
    "created_at" timestamp with time zone DEFAULT "now"() NOT NULL,
    "updated_at" timestamp with time zone DEFAULT "now"() NOT NULL,
    "model_home_id" "uuid" DEFAULT "gen_random_uuid"() NOT NULL,
    CONSTRAINT "model_homes_privacy_tier_check" CHECK (("privacy_tier" = ANY (ARRAY['sovereign'::"text", 'federated'::"text", 'public'::"text"]))),
    CONSTRAINT "model_homes_ring_check" CHECK (("ring" = ANY (ARRAY['SLM'::"text", 'LLM'::"text", 'Adapter'::"text"]))),
    CONSTRAINT "model_homes_status_check" CHECK (("status" = ANY (ARRAY['active'::"text", 'inactive'::"text", 'suspended'::"text"]))),
    CONSTRAINT "model_homes_tribunal_role_check" CHECK (("tribunal_role" = ANY (ARRAY['The Architect'::"text", 'The Guardian'::"text", 'The Mirror'::"text", 'The Philosopher'::"text", 'The Witness'::"text", 'The Weaver'::"text", 'The Steward'::"text"])))
);
...
COMMENT ON TABLE "public"."processing_runs" IS 'ManifestPipeline telemetry — one row per pipeline execution with full metrics';
...
CREATE TABLE IF NOT EXISTS "public"."provenance_links" (
    ...
    "created_at" timestamp with time zone DEFAULT "now"() NOT NULL,
    "envelope_id" "uuid",
    "related_subject_type" "text" DEFAULT ''::"text" NOT NULL,
    "related_subject_id" "text" DEFAULT ''::"text" NOT NULL,
...
ALTER TABLE "public"."signup_allowlist" ALTER COLUMN "id" ADD GENERATED BY DEFAULT AS IDENTITY (
    SEQUENCE NAME "public"."signup_allowlist_id_seq"
    START WITH 1
    INCREMENT BY 1
    NO MINVALUE
    NO MAXVALUE
...
COMMENT ON TABLE "public"."summaries" IS 'InchwormSummarizer + SnowballSummarizer output — primary, compounded, and corpus-level summaries';
...
ALTER TABLE ONLY "public"."_deprecated_build_jobs"
    ADD CONSTRAINT "build_jobs_pkey" PRIMARY KEY ("id");



ALTER TABLE ONLY "public"."_deprecated_buyers"
...
ALTER TABLE ONLY "public"."context_injection_packets"
    ADD CONSTRAINT "context_injection_packets_subject_id_packet_kind_surface_ch_key" UNIQUE ("subject_id", "packet_kind", "surface", "checksum");
...
ALTER TABLE ONLY "public"."context_injection_rules"
    ADD CONSTRAINT "context_injection_rules_subject_id_surface_source_table_sou_key" UNIQUE ("subject_id", "surface", "source_table", "source_id");
...
ALTER TABLE ONLY "public"."gate_artifacts"
    ADD CONSTRAINT "gate_artifacts_pkey" PRIMARY KEY ("id");



ALTER TABLE ONLY "public"."gate_build_jobs"
    ADD CONSTRAINT "gate_build_jobs_pkey" PRIMARY KEY ("id");



ALTER TABLE ONLY "public"."gate_buyers"
    ADD CONSTRAINT "gate_buyers_email_key" UNIQUE ("email");



ALTER TABLE ONLY "public"."gate_buyers"
    ADD CONSTRAINT "gate_buyers_pkey" PRIMARY KEY ("id");



ALTER TABLE ONLY "public"."gate_order_items"
    ADD CONSTRAINT "gate_order_items_pkey" PRIMARY KEY ("id");



ALTER TABLE ONLY "public"."gate_orders"
    ADD CONSTRAINT "gate_orders_pkey" PRIMARY KEY ("id");



ALTER TABLE ONLY "public"."gate_orders"
    ADD CONSTRAINT "gate_orders_stripe_checkout_session_id_key" UNIQUE ("stripe_checkout_session_id");



ALTER TABLE ONLY "public"."gate_package_drafts"
    ADD CONSTRAINT "gate_package_drafts_pkey" PRIMARY KEY ("id");



ALTER TABLE ONLY "public"."gate_support_requests"
    ADD CONSTRAINT "gate_support_requests_pkey" PRIMARY KEY ("id");
...
ALTER TABLE ONLY "public"."human_cognition_profiles"
    ADD CONSTRAINT "human_cognition_profiles_subject_id_key" UNIQUE ("subject_id");
...
ALTER TABLE ONLY "public"."human_consciousness_profiles"
    ADD CONSTRAINT "human_consciousness_profiles_subject_id_key" UNIQUE ("subject_id");
...
ALTER TABLE ONLY "public"."human_context_views"
    ADD CONSTRAINT "human_context_views_subject_id_scope_channel_key_key" UNIQUE ("subject_id", "scope", "channel_key");
...
ALTER TABLE ONLY "public"."human_identity_profiles"
    ADD CONSTRAINT "human_identity_profiles_subject_id_key" UNIQUE ("subject_id");
...
ALTER TABLE ONLY "public"."human_memory_records"
    ADD CONSTRAINT "human_memory_records_subject_id_content_hash_key" UNIQUE ("subject_id", "content_hash");
...
ALTER TABLE ONLY "public"."human_personality_profiles"
    ADD CONSTRAINT "human_personality_profiles_subject_id_key" UNIQUE ("subject_id");
...
ALTER TABLE ONLY "public"."human_relationship_edges"
    ADD CONSTRAINT "human_relationship_edges_subject_id_related_subject_id_rela_key" UNIQUE ("subject_id", "related_subject_id", "relationship_type");



ALTER TABLE ONLY "public"."identity_claims"
    ADD CONSTRAINT "identity_claims_pkey" PRIMARY KEY ("id");
...
ALTER TABLE ONLY "public"."identity_subjects"
    ADD CONSTRAINT "identity_subjects_agent_id_key" UNIQUE ("agent_id");



ALTER TABLE ONLY "public"."identity_subjects"
    ADD CONSTRAINT "identity_subjects_app_user_id_key" UNIQUE ("app_user_id");



ALTER TABLE ONLY "public"."identity_subjects"
    ADD CONSTRAINT "identity_subjects_auth_user_id_key" UNIQUE ("auth_user_id");
...
ALTER TABLE ONLY "public"."kv_store_441770b7"
    ADD CONSTRAINT "kv_store_441770b7_pkey" PRIMARY KEY ("key");
...
ALTER TABLE ONLY "public"."model_homes"
    ADD CONSTRAINT "model_homes_pkey" PRIMARY KEY ("home_id");
...
ALTER TABLE ONLY "public"."_deprecated_order_items"
...
ALTER TABLE ONLY "public"."_deprecated_package_drafts"
    ADD CONSTRAINT "package_drafts_pkey" PRIMARY KEY ("id");
...
ALTER TABLE ONLY "public"."profile_pipeline_run_links"
    ADD CONSTRAINT "profile_pipeline_run_links_pkey" PRIMARY KEY ("id");



ALTER TABLE ONLY "public"."profile_pipeline_run_links"
    ADD CONSTRAINT "profile_pipeline_run_links_run_id_link_role_linked_table_li_key" UNIQUE ("run_id", "link_role", "linked_table", "linked_id");
...
ALTER TABLE ONLY "public"."provenance_envelopes"
    ADD CONSTRAINT "provenance_envelopes_pkey" PRIMARY KEY ("id");
...
ALTER TABLE ONLY "public"."rate_limit_buckets"
    ADD CONSTRAINT "rate_limit_buckets_user_key_action_type_window_start_key" UNIQUE ("user_key", "action_type", "window_start");
...
ALTER TABLE ONLY "public"."scaffold_nodes"
    ADD CONSTRAINT "scaffold_nodes_pkey" PRIMARY KEY ("id");
...
ALTER TABLE ONLY "public"."signup_allowlist"
    ADD CONSTRAINT "signup_allowlist_email_key" UNIQUE ("email");
...
ALTER TABLE ONLY "public"."_deprecated_support_requests"
...
CREATE INDEX "artifacts_blueprint_idx" ON "public"."_deprecated_artifacts" USING "btree" ("blueprint_id");



CREATE INDEX "artifacts_owner_created_idx" ON "public"."_deprecated_artifacts" USING "btree" ("owner_user_id", "created_at" DESC);



CREATE INDEX "artifacts_owner_status_idx" ON "public"."_deprecated_artifacts" USING "btree" ("owner_user_id", "status", "created_at" DESC);



CREATE INDEX "artifacts_source_capture_ids_gin" ON "public"."_deprecated_artifacts" USING "gin" ("source_capture_ids");



CREATE INDEX "artifacts_source_scaffold_node_ids_gin" ON "public"."_deprecated_artifacts" USING "gin" ("source_scaffold_node_ids");



CREATE INDEX "artifacts_tags_gin" ON "public"."_deprecated_artifacts" USING "gin" ("tags");
...
CREATE INDEX "bucket_drops_module_key_idx" ON "public"."bucket_drops" USING "btree" ("module_key");



CREATE INDEX "bucket_drops_promoted_at_idx" ON "public"."bucket_drops" USING "btree" ("promoted_at");
...
CREATE INDEX "bucket_drops_stage_idx" ON "public"."bucket_drops" USING "btree" ("stage");
...
CREATE INDEX "bucket_drops_user_id_idx" ON "public"."bucket_drops" USING "btree" ("user_id");



CREATE UNIQUE INDEX "capture_events_owner_content_hash_key" ON "public"."capture_events" USING "btree" ("owner_user_id", "content_hash") WHERE ("content_hash" IS NOT NULL);



CREATE INDEX "capture_events_owner_created_idx" ON "public"."capture_events" USING "btree" ("owner_user_id", "created_at" DESC);



CREATE INDEX "capture_events_preservation_idx" ON "public"."capture_events" USING "btree" ("owner_user_id", "preservation_status", "created_at" DESC);
...
CREATE INDEX "capture_events_room_type_idx" ON "public"."capture_events" USING "btree" ("source_room", "capture_type", "created_at" DESC);
...
CREATE INDEX "codex_jobs_artifact_id_idx" ON "public"."codex_jobs" USING "btree" ("artifact_id", "created_at");
...
CREATE INDEX "embeddings_hnsw_idx" ON "public"."embeddings" USING "hnsw" ("embedding" "extensions"."vector_cosine_ops");
...
CREATE INDEX "embodiment_profiles_review_state_idx" ON "public"."embodiment_profiles" USING "btree" ("review_state", "updated_at" DESC);



CREATE UNIQUE INDEX "embodiment_profiles_source_checksum_key" ON "public"."embodiment_profiles" USING "btree" ("source_checksum") WHERE ("source_checksum" IS NOT NULL);
...
CREATE INDEX "gate_artifacts_build_job_id_idx" ON "public"."gate_artifacts" USING "btree" ("build_job_id");



CREATE INDEX "gate_artifacts_download_token_idx" ON "public"."gate_artifacts" USING "btree" ("download_token");



CREATE INDEX "gate_build_jobs_order_id_idx" ON "public"."gate_build_jobs" USING "btree" ("order_id");



CREATE INDEX "gate_build_jobs_status_idx" ON "public"."gate_build_jobs" USING "btree" ("status");



CREATE INDEX "gate_orders_buyer_id_idx" ON "public"."gate_orders" USING "btree" ("buyer_id");



CREATE INDEX "gate_orders_order_status_idx" ON "public"."gate_orders" USING "btree" ("order_status");



CREATE INDEX "gate_orders_package_draft_id_idx" ON "public"."gate_orders" USING "btree" ("package_draft_id");



CREATE INDEX "gate_orders_payment_status_idx" ON "public"."gate_orders" USING "btree" ("payment_status");



CREATE INDEX "gate_package_drafts_buyer_email_idx" ON "public"."gate_package_drafts" USING "btree" ("buyer_email");



CREATE INDEX "gate_package_drafts_buyer_id_idx" ON "public"."gate_package_drafts" USING "btree" ("buyer_id");



CREATE INDEX "gate_package_drafts_config_hash_idx" ON "public"."gate_package_drafts" USING "btree" ("config_hash");



CREATE INDEX "gate_package_drafts_status_idx" ON "public"."gate_package_drafts" USING "btree" ("status");



CREATE INDEX "gate_support_requests_order_id_idx" ON "public"."gate_support_requests" USING "btree" ("order_id");



CREATE INDEX "gate_support_requests_package_draft_id_idx" ON "public"."gate_support_requests" USING "btree" ("package_draft_id");
...
CREATE INDEX "human_memory_records_embedding_idx" ON "public"."human_memory_records" USING "ivfflat" ("embedding" "extensions"."vector_cosine_ops") WITH ("lists"='100');
...
CREATE INDEX "identity_claims_claim_type_idx" ON "public"."identity_claims" USING "btree" ("owner_user_id", "claim_type", "created_at" DESC);



CREATE INDEX "identity_claims_owner_status_idx" ON "public"."identity_claims" USING "btree" ("owner_user_id", "status", "created_at" DESC);



CREATE INDEX "identity_claims_support_artifact_ids_gin" ON "public"."identity_claims" USING "gin" ("support_artifact_ids");



CREATE INDEX "identity_claims_support_capture_ids_gin" ON "public"."identity_claims" USING "gin" ("support_capture_ids");



CREATE INDEX "identity_claims_support_scaffold_node_ids_gin" ON "public"."identity_claims" USING "gin" ("support_scaffold_node_ids");
...
CREATE INDEX "idx_artifacts_build_job_id" ON "public"."_deprecated_artifacts" USING "btree" ("build_job_id");



CREATE UNIQUE INDEX "idx_bucket_drops_promoted_memory_id_unique" ON "public"."bucket_drops" USING "btree" ("promoted_memory_id") WHERE ("promoted_memory_id" IS NOT NULL);



CREATE INDEX "idx_bucket_drops_stage" ON "public"."bucket_drops" USING "btree" ("stage", "created_at");
...
CREATE INDEX "idx_bucket_drops_subject_id" ON "public"."bucket_drops" USING "btree" ("subject_id") WHERE ("subject_id" IS NOT NULL);



CREATE INDEX "idx_build_jobs_order_id" ON "public"."_deprecated_build_jobs" USING "btree" ("order_id");



CREATE INDEX "idx_build_jobs_status" ON "public"."_deprecated_build_jobs" USING "btree" ("status");
...
CREATE INDEX "idx_documents_temporal_period" ON "public"."documents" USING "btree" ("temporal_period");



CREATE INDEX "idx_documents_timeline_folder" ON "public"."documents" USING "btree" ("timeline_folder");



CREATE INDEX "idx_fragments_temporal_period" ON "public"."knowledge_fragments" USING "btree" ("temporal_period");



CREATE INDEX "idx_fragments_timeline_folder" ON "public"."knowledge_fragments" USING "btree" ("timeline_folder");
...
CREATE INDEX "idx_kf_document_id" ON "public"."knowledge_fragments" USING "btree" ("document_id");



CREATE INDEX "idx_kf_file_last_modified" ON "public"."knowledge_fragments" USING "btree" ("file_last_modified");



CREATE INDEX "idx_kf_ingested_at" ON "public"."knowledge_fragments" USING "btree" ("ingested_at");



CREATE INDEX "idx_kf_package" ON "public"."knowledge_fragments" USING "btree" ("package");



CREATE INDEX "idx_kf_source_path" ON "public"."knowledge_fragments" USING "btree" ("source_path");



CREATE INDEX "idx_loom_annotations_run_id" ON "public"."loom_annotations" USING "btree" ("run_id");



CREATE INDEX "idx_loom_annotations_type" ON "public"."loom_annotations" USING "btree" ("type");
...
CREATE INDEX "idx_memory_entries_embedding" ON "public"."memory_entries" USING "ivfflat" ("embedding" "extensions"."vector_cosine_ops") WITH ("lists"='100');



CREATE INDEX "idx_orders_buyer_id" ON "public"."_deprecated_orders" USING "btree" ("buyer_id");



CREATE INDEX "idx_orders_order_status" ON "public"."_deprecated_orders" USING "btree" ("order_status");



CREATE INDEX "idx_orders_package_draft_id" ON "public"."_deprecated_orders" USING "btree" ("package_draft_id");



CREATE INDEX "idx_package_drafts_buyer_email" ON "public"."_deprecated_package_drafts" USING "btree" ("buyer_email");



CREATE INDEX "idx_package_drafts_buyer_id" ON "public"."_deprecated_package_drafts" USING "btree" ("buyer_id");



CREATE INDEX "idx_package_drafts_config_hash" ON "public"."_deprecated_package_drafts" USING "btree" ("config_hash");



CREATE INDEX "idx_package_drafts_status" ON "public"."_deprecated_package_drafts" USING "btree" ("status");



CREATE INDEX "idx_rlb_user_key_window" ON "public"."rate_limit_buckets" USING "btree" ("user_key", "window_start");
...
CREATE INDEX "idx_summaries_level" ON "public"."summaries" USING "btree" ("level");



CREATE INDEX "idx_summaries_run_id" ON "public"."summaries" USING "btree" ("run_id");



CREATE INDEX "idx_support_requests_order_id" ON "public"."_deprecated_support_requests" USING "btree" ("order_id");



CREATE INDEX "idx_support_requests_package_draft_id" ON "public"."_deprecated_support_requests" USING "btree" ("package_draft_id");
...
CREATE INDEX "knowledge_asset_chunks_embedding_idx" ON "public"."knowledge_asset_chunks" USING "hnsw" ("embedding" "extensions"."vector_cosine_ops") WHERE ("embedding" IS NOT NULL);
...
CREATE INDEX "knowledge_fragments_content_hash_idx" ON "public"."knowledge_fragments" USING "btree" ("content_hash");
...
CREATE INDEX "knowledge_fragments_document_id_idx" ON "public"."knowledge_fragments" USING "btree" ("document_id");



CREATE INDEX "knowledge_fragments_embedding_idx" ON "public"."knowledge_fragments" USING "hnsw" ("embedding" "extensions"."vector_cosine_ops") WITH ("m"='16', "ef_construction"='64');



CREATE INDEX "knowledge_fragments_run_id_idx" ON "public"."knowledge_fragments" USING "btree" ("run_id");
...
CREATE INDEX "knowledge_fragments_source_file_idx" ON "public"."knowledge_fragments" USING "btree" ("source_file");
...
CREATE INDEX "kv_store_441770b7_key_idx" ON "public"."kv_store_441770b7" USING "btree" ("key" "text_pattern_ops");
...
CREATE INDEX "profile_pipeline_run_links_run_idx" ON "public"."profile_pipeline_run_links" USING "btree" ("run_id", "created_at" DESC);



CREATE INDEX "profile_pipeline_runs_kind_status_idx" ON "public"."profile_pipeline_runs" USING "btree" ("run_kind", "status", "created_at" DESC);



CREATE INDEX "profile_pipeline_runs_owner_created_idx" ON "public"."profile_pipeline_runs" USING "btree" ("owner_user_id", "created_at" DESC);
...
CREATE UNIQUE INDEX "provenance_content_hash_idx" ON "public"."provenance_envelopes" USING "btree" ("content_hash");
...
CREATE INDEX "scaffold_nodes_owner_kind_idx" ON "public"."scaffold_nodes" USING "btree" ("owner_user_id", "node_kind", "created_at" DESC);



CREATE INDEX "scaffold_nodes_owner_state_idx" ON "public"."scaffold_nodes" USING "btree" ("owner_user_id", "state", "created_at" DESC);



CREATE INDEX "scaffold_nodes_source_artifact_ids_gin" ON "public"."scaffold_nodes" USING "gin" ("source_artifact_ids");



CREATE INDEX "scaffold_nodes_source_capture_ids_gin" ON "public"."scaffold_nodes" USING "gin" ("source_capture_ids");
...
CREATE INDEX "skill_fragments_content_fts" ON "public"."skill_fragments" USING "gin" ("to_tsvector"('"english"'::"regconfig", "content"));
...
CREATE INDEX "skill_fragments_embedding_idx" ON "public"."skill_fragments" USING "hnsw" ("embedding" "extensions"."vector_cosine_ops") WITH ("m"='16', "ef_construction"='64');



CREATE INDEX "skill_fragments_skill_name_idx" ON "public"."skill_fragments" USING "btree" ("skill_name") WHERE ("embedding" IS NOT NULL);
...
CREATE OR REPLACE TRIGGER "trg_artifacts_touch_updated_at" BEFORE UPDATE ON "public"."_deprecated_artifacts" FOR EACH ROW EXECUTE FUNCTION "public"."gv_touch_updated_at"();
...
CREATE OR REPLACE TRIGGER "trg_capture_events_touch_updated_at" BEFORE UPDATE ON "public"."capture_events" FOR EACH ROW EXECUTE FUNCTION "public"."gv_touch_updated_at"();
...
CREATE OR REPLACE TRIGGER "trg_corpus_harvest_on_complete" AFTER UPDATE ON "public"."profile_pipeline_runs" FOR EACH ROW EXECUTE FUNCTION "public"."fn_corpus_harvest_on_pipeline_complete"();



CREATE OR REPLACE TRIGGER "trg_gate_orders_set_updated_at" BEFORE UPDATE ON "public"."gate_orders" FOR EACH ROW EXECUTE FUNCTION "public"."set_updated_at_timestamp"();



CREATE OR REPLACE TRIGGER "trg_gate_package_drafts_set_updated_at" BEFORE UPDATE ON "public"."gate_package_drafts" FOR EACH ROW EXECUTE FUNCTION "public"."set_updated_at_timestamp"();
...
CREATE OR REPLACE TRIGGER "trg_identity_claims_touch_updated_at" BEFORE UPDATE ON "public"."identity_claims" FOR EACH ROW EXECUTE FUNCTION "public"."gv_touch_updated_at"();
...
CREATE OR REPLACE TRIGGER "trg_orders_set_updated_at" BEFORE UPDATE ON "public"."_deprecated_orders" FOR EACH ROW EXECUTE FUNCTION "public"."set_updated_at_timestamp"();



CREATE OR REPLACE TRIGGER "trg_package_drafts_set_updated_at" BEFORE UPDATE ON "public"."_deprecated_package_drafts" FOR EACH ROW EXECUTE FUNCTION "public"."set_updated_at_timestamp"();



CREATE OR REPLACE TRIGGER "trg_profile_pipeline_runs_touch_updated_at" BEFORE UPDATE ON "public"."profile_pipeline_runs" FOR EACH ROW EXECUTE FUNCTION "public"."gv_touch_updated_at"();
...
CREATE OR REPLACE TRIGGER "trg_scaffold_nodes_touch_updated_at" BEFORE UPDATE ON "public"."scaffold_nodes" FOR EACH ROW EXECUTE FUNCTION "public"."gv_touch_updated_at"();
...
ALTER TABLE ONLY "public"."app_users"
    ADD CONSTRAINT "app_users_subject_id_fkey" FOREIGN KEY ("subject_id") REFERENCES "public"."identity_subjects"("subject_id") ON DELETE SET NULL;
...
ALTER TABLE ONLY "public"."_deprecated_artifacts"
    ADD CONSTRAINT "artifacts_blueprint_id_fkey" FOREIGN KEY ("blueprint_id") REFERENCES "public"."blueprints"("id") ON DELETE SET NULL;



ALTER TABLE ONLY "public"."_deprecated_artifacts"
    ADD CONSTRAINT "artifacts_build_job_id_fkey" FOREIGN KEY ("build_job_id") REFERENCES "public"."_deprecated_build_jobs"("id") ON DELETE CASCADE;



ALTER TABLE ONLY "public"."_deprecated_artifacts"
    ADD CONSTRAINT "artifacts_owner_user_id_fkey" FOREIGN KEY ("owner_user_id") REFERENCES "auth"."users"("id") ON DELETE CASCADE;



ALTER TABLE ONLY "public"."_deprecated_artifacts"
    ADD CONSTRAINT "artifacts_source_file_id_fkey" FOREIGN KEY ("source_file_id") REFERENCES "public"."user_files"("id") ON DELETE SET NULL;
...
ALTER TABLE ONLY "public"."_deprecated_build_jobs"
    ADD CONSTRAINT "build_jobs_order_id_fkey" FOREIGN KEY ("order_id") REFERENCES "public"."_deprecated_orders"("id") ON DELETE CASCADE;



ALTER TABLE ONLY "public"."_deprecated_build_jobs"
    ADD CONSTRAINT "build_jobs_package_draft_id_fkey" FOREIGN KEY ("package_draft_id") REFERENCES "public"."_deprecated_package_drafts"("id") ON DELETE CASCADE;
...
ALTER TABLE ONLY "public"."capture_events"
    ADD CONSTRAINT "capture_events_owner_user_id_fkey" FOREIGN KEY ("owner_user_id") REFERENCES "auth"."users"("id") ON DELETE CASCADE;



ALTER TABLE ONLY "public"."capture_events"
    ADD CONSTRAINT "capture_events_source_bucket_drop_id_fkey" FOREIGN KEY ("source_bucket_drop_id") REFERENCES "public"."bucket_drops"("id") ON DELETE SET NULL;
...
ALTER TABLE ONLY "public"."consciousness_profiles"
    ADD CONSTRAINT "consciousness_profiles_auth_user_id_fkey" FOREIGN KEY ("auth_user_id") REFERENCES "auth"."users"("id") ON DELETE SET NULL;



ALTER TABLE ONLY "public"."consciousness_profiles"
    ADD CONSTRAINT "consciousness_profiles_subject_id_fkey" FOREIGN KEY ("subject_id") REFERENCES "public"."identity_subjects"("subject_id") ON DELETE SET NULL;
...
ALTER TABLE ONLY "public"."embodiment_profiles"
    ADD CONSTRAINT "embodiment_profiles_approved_by_fkey" FOREIGN KEY ("approved_by") REFERENCES "auth"."users"("id") ON DELETE SET NULL;
...
ALTER TABLE ONLY "public"."founder_context"
    ADD CONSTRAINT "founder_context_subject_id_fkey" FOREIGN KEY ("subject_id") REFERENCES "public"."identity_subjects"("subject_id") ON DELETE SET NULL;
...
ALTER TABLE ONLY "public"."gate_artifacts"
    ADD CONSTRAINT "gate_artifacts_build_job_id_fkey" FOREIGN KEY ("build_job_id") REFERENCES "public"."gate_build_jobs"("id") ON DELETE CASCADE;



ALTER TABLE ONLY "public"."gate_build_jobs"
    ADD CONSTRAINT "gate_build_jobs_order_id_fkey" FOREIGN KEY ("order_id") REFERENCES "public"."gate_orders"("id") ON DELETE CASCADE;



ALTER TABLE ONLY "public"."gate_build_jobs"
    ADD CONSTRAINT "gate_build_jobs_package_draft_id_fkey" FOREIGN KEY ("package_draft_id") REFERENCES "public"."gate_package_drafts"("id") ON DELETE CASCADE;



ALTER TABLE ONLY "public"."gate_order_items"
    ADD CONSTRAINT "gate_order_items_order_id_fkey" FOREIGN KEY ("order_id") REFERENCES "public"."gate_orders"("id") ON DELETE CASCADE;



ALTER TABLE ONLY "public"."gate_orders"
    ADD CONSTRAINT "gate_orders_buyer_id_fkey" FOREIGN KEY ("buyer_id") REFERENCES "public"."gate_buyers"("id") ON DELETE RESTRICT;



ALTER TABLE ONLY "public"."gate_orders"
    ADD CONSTRAINT "gate_orders_package_draft_id_fkey" FOREIGN KEY ("package_draft_id") REFERENCES "public"."gate_package_drafts"("id") ON DELETE RESTRICT;



ALTER TABLE ONLY "public"."gate_package_drafts"
    ADD CONSTRAINT "gate_package_drafts_buyer_id_fkey" FOREIGN KEY ("buyer_id") REFERENCES "public"."gate_buyers"("id") ON DELETE CASCADE;



ALTER TABLE ONLY "public"."gate_support_requests"
    ADD CONSTRAINT "gate_support_requests_order_id_fkey" FOREIGN KEY ("order_id") REFERENCES "public"."gate_orders"("id") ON DELETE SET NULL;



ALTER TABLE ONLY "public"."gate_support_requests"
    ADD CONSTRAINT "gate_support_requests_package_draft_id_fkey" FOREIGN KEY ("package_draft_id") REFERENCES "public"."gate_package_drafts"("id") ON DELETE SET NULL;
...
ALTER TABLE ONLY "public"."human_cognition_profiles"
    ADD CONSTRAINT "human_cognition_profiles_auth_user_id_fkey" FOREIGN KEY ("auth_user_id") REFERENCES "auth"."users"("id") ON DELETE CASCADE;



ALTER TABLE ONLY "public"."human_cognition_profiles"
    ADD CONSTRAINT "human_cognition_profiles_subject_id_fkey" FOREIGN KEY ("subject_id") REFERENCES "public"."identity_subjects"("subject_id") ON DELETE CASCADE;



ALTER TABLE ONLY "public"."human_consciousness_profiles"
    ADD CONSTRAINT "human_consciousness_profiles_auth_user_id_fkey" FOREIGN KEY ("auth_user_id") REFERENCES "auth"."users"("id") ON DELETE CASCADE;



ALTER TABLE ONLY "public"."human_consciousness_profiles"
    ADD CONSTRAINT "human_consciousness_profiles_subject_id_fkey" FOREIGN KEY ("subject_id") REFERENCES "public"."identity_subjects"("subject_id") ON DELETE CASCADE;



ALTER TABLE ONLY "public"."human_context_views"
    ADD CONSTRAINT "human_context_views_auth_user_id_fkey" FOREIGN KEY ("auth_user_id") REFERENCES "auth"."users"("id") ON DELETE CASCADE;



ALTER TABLE ONLY "public"."human_context_views"
    ADD CONSTRAINT "human_context_views_relationship_subject_id_fkey" FOREIGN KEY ("relationship_subject_id") REFERENCES "public"."identity_subjects"("subject_id") ON DELETE SET NULL;



ALTER TABLE ONLY "public"."human_context_views"
    ADD CONSTRAINT "human_context_views_subject_id_fkey" FOREIGN KEY ("subject_id") REFERENCES "public"."identity_subjects"("subject_id") ON DELETE CASCADE;



ALTER TABLE ONLY "public"."human_continuity_snapshots"
    ADD CONSTRAINT "human_continuity_snapshots_auth_user_id_fkey" FOREIGN KEY ("auth_user_id") REFERENCES "auth"."users"("id") ON DELETE CASCADE;



ALTER TABLE ONLY "public"."human_continuity_snapshots"
    ADD CONSTRAINT "human_continuity_snapshots_subject_id_fkey" FOREIGN KEY ("subject_id") REFERENCES "public"."identity_subjects"("subject_id") ON DELETE CASCADE;



ALTER TABLE ONLY "public"."human_identity_evidence"
    ADD CONSTRAINT "human_identity_evidence_auth_user_id_fkey" FOREIGN KEY ("auth_user_id") REFERENCES "auth"."users"("id") ON DELETE CASCADE;



ALTER TABLE ONLY "public"."human_identity_evidence"
    ADD CONSTRAINT "human_identity_evidence_source_asset_id_fkey" FOREIGN KEY ("source_asset_id") REFERENCES "public"."knowledge_assets"("id") ON DELETE SET NULL;



ALTER TABLE ONLY "public"."human_identity_evidence"
    ADD CONSTRAINT "human_identity_evidence_source_memory_entry_id_fkey" FOREIGN KEY ("source_memory_entry_id") REFERENCES "public"."memory_entries"("id") ON DELETE SET NULL;



ALTER TABLE ONLY "public"."human_identity_evidence"
    ADD CONSTRAINT "human_identity_evidence_subject_id_fkey" FOREIGN KEY ("subject_id") REFERENCES "public"."identity_subjects"("subject_id") ON DELETE CASCADE;



ALTER TABLE ONLY "public"."human_identity_mutations"
    ADD CONSTRAINT "human_identity_mutations_auth_user_id_fkey" FOREIGN KEY ("auth_user_id") REFERENCES "auth"."users"("id") ON DELETE CASCADE;



ALTER TABLE ONLY "public"."human_identity_mutations"
    ADD CONSTRAINT "human_identity_mutations_proposed_by_user_id_fkey" FOREIGN KEY ("proposed_by_user_id") REFERENCES "auth"."users"("id") ON DELETE SET NULL;



ALTER TABLE ONLY "public"."human_identity_mutations"
    ADD CONSTRAINT "human_identity_mutations_source_asset_id_fkey" FOREIGN KEY ("source_asset_id") REFERENCES "public"."knowledge_assets"("id") ON DELETE SET NULL;



ALTER TABLE ONLY "public"."human_identity_mutations"
    ADD CONSTRAINT "human_identity_mutations_subject_id_fkey" FOREIGN KEY ("subject_id") REFERENCES "public"."identity_subjects"("subject_id") ON DELETE CASCADE;



ALTER TABLE ONLY "public"."human_identity_profiles"
    ADD CONSTRAINT "human_identity_profiles_auth_user_id_fkey" FOREIGN KEY ("auth_user_id") REFERENCES "auth"."users"("id") ON DELETE CASCADE;



ALTER TABLE ONLY "public"."human_identity_profiles"
    ADD CONSTRAINT "human_identity_profiles_subject_id_fkey" FOREIGN KEY ("subject_id") REFERENCES "public"."identity_subjects"("subject_id") ON DELETE CASCADE;



ALTER TABLE ONLY "public"."human_identity_review_events"
    ADD CONSTRAINT "human_identity_review_events_auth_user_id_fkey" FOREIGN KEY ("auth_user_id") REFERENCES "auth"."users"("id") ON DELETE SET NULL;
...
ALTER TABLE ONLY "public"."human_identity_review_events"
    ADD CONSTRAINT "human_identity_review_events_reviewer_user_id_fkey" FOREIGN KEY ("reviewer_user_id") REFERENCES "auth"."users"("id") ON DELETE SET NULL;



ALTER TABLE ONLY "public"."human_identity_review_events"
    ADD CONSTRAINT "human_identity_review_events_subject_id_fkey" FOREIGN KEY ("subject_id") REFERENCES "public"."identity_subjects"("subject_id") ON DELETE SET NULL;



ALTER TABLE ONLY "public"."human_identity_rollback_events"
    ADD CONSTRAINT "human_identity_rollback_events_auth_user_id_fkey" FOREIGN KEY ("auth_user_id") REFERENCES "auth"."users"("id") ON DELETE SET NULL;
...
ALTER TABLE ONLY "public"."human_identity_rollback_events"
    ADD CONSTRAINT "human_identity_rollback_events_rolled_back_by_fkey" FOREIGN KEY ("rolled_back_by") REFERENCES "auth"."users"("id") ON DELETE SET NULL;



ALTER TABLE ONLY "public"."human_identity_rollback_events"
    ADD CONSTRAINT "human_identity_rollback_events_subject_id_fkey" FOREIGN KEY ("subject_id") REFERENCES "public"."identity_subjects"("subject_id") ON DELETE SET NULL;



ALTER TABLE ONLY "public"."human_memory_records"
    ADD CONSTRAINT "human_memory_records_auth_user_id_fkey" FOREIGN KEY ("auth_user_id") REFERENCES "auth"."users"("id") ON DELETE CASCADE;



ALTER TABLE ONLY "public"."human_memory_records"
    ADD CONSTRAINT "human_memory_records_source_asset_id_fkey" FOREIGN KEY ("source_asset_id") REFERENCES "public"."knowledge_assets"("id") ON DELETE SET NULL;



ALTER TABLE ONLY "public"."human_memory_records"
    ADD CONSTRAINT "human_memory_records_source_memory_entry_id_fkey" FOREIGN KEY ("source_memory_entry_id") REFERENCES "public"."memory_entries"("id") ON DELETE SET NULL;



ALTER TABLE ONLY "public"."human_memory_records"
    ADD CONSTRAINT "human_memory_records_subject_id_fkey" FOREIGN KEY ("subject_id") REFERENCES "public"."identity_subjects"("subject_id") ON DELETE CASCADE;



ALTER TABLE ONLY "public"."human_personality_profiles"
    ADD CONSTRAINT "human_personality_profiles_auth_user_id_fkey" FOREIGN KEY ("auth_user_id") REFERENCES "auth"."users"("id") ON DELETE CASCADE;



ALTER TABLE ONLY "public"."human_personality_profiles"
    ADD CONSTRAINT "human_personality_profiles_subject_id_fkey" FOREIGN KEY ("subject_id") REFERENCES "public"."identity_subjects"("subject_id") ON DELETE CASCADE;



ALTER TABLE ONLY "public"."human_relationship_edges"
    ADD CONSTRAINT "human_relationship_edges_auth_user_id_fkey" FOREIGN KEY ("auth_user_id") REFERENCES "auth"."users"("id") ON DELETE CASCADE;



ALTER TABLE ONLY "public"."human_relationship_edges"
    ADD CONSTRAINT "human_relationship_edges_related_subject_id_fkey" FOREIGN KEY ("related_subject_id") REFERENCES "public"."identity_subjects"("subject_id") ON DELETE CASCADE;



ALTER TABLE ONLY "public"."human_relationship_edges"
    ADD CONSTRAINT "human_relationship_edges_subject_id_fkey" FOREIGN KEY ("subject_id") REFERENCES "public"."identity_subjects"("subject_id") ON DELETE CASCADE;



ALTER TABLE ONLY "public"."identity_claims"
    ADD CONSTRAINT "identity_claims_owner_user_id_fkey" FOREIGN KEY ("owner_user_id") REFERENCES "auth"."users"("id") ON DELETE CASCADE;



ALTER TABLE ONLY "public"."identity_claims"
    ADD CONSTRAINT "identity_claims_reviewer_user_id_fkey" FOREIGN KEY ("reviewer_user_id") REFERENCES "auth"."users"("id") ON DELETE SET NULL;
...
ALTER TABLE ONLY "public"."identity_subjects"
    ADD CONSTRAINT "identity_subjects_agent_id_fkey" FOREIGN KEY ("agent_id") REFERENCES "public"."agents"("agent_id") ON DELETE CASCADE;



ALTER TABLE ONLY "public"."identity_subjects"
    ADD CONSTRAINT "identity_subjects_app_user_id_fkey" FOREIGN KEY ("app_user_id") REFERENCES "public"."app_users"("id") ON DELETE CASCADE;



ALTER TABLE ONLY "public"."identity_subjects"
    ADD CONSTRAINT "identity_subjects_auth_user_id_fkey" FOREIGN KEY ("auth_user_id") REFERENCES "auth"."users"("id") ON DELETE CASCADE;
...
ALTER TABLE ONLY "public"."knowledge_fragments"
    ADD CONSTRAINT "knowledge_fragments_run_id_fkey" FOREIGN KEY ("run_id") REFERENCES "public"."processing_runs"("run_id");
...
ALTER TABLE ONLY "public"."memory_entries"
    ADD CONSTRAINT "memory_entries_auth_user_id_fkey" FOREIGN KEY ("auth_user_id") REFERENCES "auth"."users"("id") ON DELETE SET NULL;



ALTER TABLE ONLY "public"."memory_entries"
    ADD CONSTRAINT "memory_entries_source_asset_id_fkey" FOREIGN KEY ("source_asset_id") REFERENCES "public"."knowledge_assets"("id") ON DELETE SET NULL;



ALTER TABLE ONLY "public"."memory_entries"
    ADD CONSTRAINT "memory_entries_subject_id_fkey" FOREIGN KEY ("subject_id") REFERENCES "public"."identity_subjects"("subject_id") ON DELETE SET NULL;
...
ALTER TABLE ONLY "public"."model_homes"
    ADD CONSTRAINT "model_homes_model_id_fkey" FOREIGN KEY ("model_id") REFERENCES "public"."models"("model_id");



ALTER TABLE ONLY "public"."model_homes"
    ADD CONSTRAINT "model_homes_provider_id_fkey" FOREIGN KEY ("provider_id") REFERENCES "public"."model_providers"("provider_id");
...
ALTER TABLE ONLY "public"."_deprecated_order_items"
    ADD CONSTRAINT "order_items_order_id_fkey" FOREIGN KEY ("order_id") REFERENCES "public"."_deprecated_orders"("id") ON DELETE CASCADE;



ALTER TABLE ONLY "public"."_deprecated_orders"
    ADD CONSTRAINT "orders_buyer_id_fkey" FOREIGN KEY ("buyer_id") REFERENCES "public"."_deprecated_buyers"("id") ON DELETE RESTRICT;



ALTER TABLE ONLY "public"."_deprecated_orders"
    ADD CONSTRAINT "orders_package_draft_id_fkey" FOREIGN KEY ("package_draft_id") REFERENCES "public"."_deprecated_package_drafts"("id") ON DELETE RESTRICT;



ALTER TABLE ONLY "public"."_deprecated_package_drafts"
    ADD CONSTRAINT "package_drafts_buyer_id_fkey" FOREIGN KEY ("buyer_id") REFERENCES "public"."_deprecated_buyers"("id") ON DELETE CASCADE;
...
ALTER TABLE ONLY "public"."profile_pipeline_runs"
    ADD CONSTRAINT "profile_pipeline_runs_owner_user_id_fkey" FOREIGN KEY ("owner_user_id") REFERENCES "auth"."users"("id") ON DELETE CASCADE;



ALTER TABLE ONLY "public"."profile_pipeline_runs"
    ADD CONSTRAINT "profile_pipeline_runs_parent_run_id_fkey" FOREIGN KEY ("parent_run_id") REFERENCES "public"."profile_pipeline_runs"("run_id") ON DELETE SET NULL;
...
ALTER TABLE ONLY "public"."provenance_links"
    ADD CONSTRAINT "provenance_links_child_envelope_id_fkey" FOREIGN KEY ("child_envelope_id") REFERENCES "public"."provenance_envelopes"("id");
...
ALTER TABLE ONLY "public"."provenance_links"
    ADD CONSTRAINT "provenance_links_parent_envelope_id_fkey" FOREIGN KEY ("parent_envelope_id") REFERENCES "public"."provenance_envelopes"("id");
...
ALTER TABLE ONLY "public"."route_embodiment_assignments"
    ADD CONSTRAINT "route_embodiment_assignments_embodiment_profile_slug_fkey" FOREIGN KEY ("embodiment_profile_slug") REFERENCES "public"."embodiment_profiles"("slug") ON DELETE RESTRICT;



ALTER TABLE ONLY "public"."route_embodiment_assignments"
    ADD CONSTRAINT "route_embodiment_assignments_profile_slug_fk" FOREIGN KEY ("embodiment_profile_slug") REFERENCES "public"."embodiment_profiles"("slug") ON DELETE RESTRICT;



ALTER TABLE ONLY "public"."scaffold_nodes"
    ADD CONSTRAINT "scaffold_nodes_owner_user_id_fkey" FOREIGN KEY ("owner_user_id") REFERENCES "auth"."users"("id") ON DELETE CASCADE;



ALTER TABLE ONLY "public"."scaffold_nodes"
    ADD CONSTRAINT "scaffold_nodes_reviewed_by_fkey" FOREIGN KEY ("reviewed_by") REFERENCES "auth"."users"("id") ON DELETE SET NULL;
...
ALTER TABLE ONLY "public"."skill_fragments"
    ADD CONSTRAINT "skill_fragments_document_id_fkey" FOREIGN KEY ("document_id") REFERENCES "public"."documents"("document_id") ON DELETE CASCADE;
...
ALTER TABLE ONLY "public"."_deprecated_support_requests"
    ADD CONSTRAINT "support_requests_order_id_fkey" FOREIGN KEY ("order_id") REFERENCES "public"."_deprecated_orders"("id") ON DELETE SET NULL;



ALTER TABLE ONLY "public"."_deprecated_support_requests"
    ADD CONSTRAINT "support_requests_package_draft_id_fkey" FOREIGN KEY ("package_draft_id") REFERENCES "public"."_deprecated_package_drafts"("id") ON DELETE SET NULL;
...
CREATE POLICY "Public read active route embodiment assignments" ON "public"."route_embodiment_assignments" FOR SELECT USING (true);
...
CREATE POLICY "Service role full access" ON "public"."skill_fragments" TO "service_role" USING (true) WITH CHECK (true);
...
CREATE POLICY "Service role full access artifacts" ON "public"."_deprecated_artifacts" TO "service_role" USING (true) WITH CHECK (true);



CREATE POLICY "Service role full access capture_events" ON "public"."capture_events" TO "service_role" USING (true) WITH CHECK (true);
...
CREATE POLICY "Service role full access identity_claims" ON "public"."identity_claims" TO "service_role" USING (true) WITH CHECK (true);
...
CREATE POLICY "Service role full access on session_rate_limits" ON "public"."session_rate_limits" USING (("auth"."role"() = 'service_role'::"text"));



CREATE POLICY "Service role full access on users" ON "public"."users" USING (("auth"."role"() = 'service_role'::"text"));
...
CREATE POLICY "Service role full access profile_pipeline_run_links" ON "public"."profile_pipeline_run_links" TO "service_role" USING (true) WITH CHECK (true);



CREATE POLICY "Service role full access profile_pipeline_runs" ON "public"."profile_pipeline_runs" TO "service_role" USING (true) WITH CHECK (true);



CREATE POLICY "Service role full access route embodiment assignments" ON "public"."route_embodiment_assignments" TO "service_role" USING (true) WITH CHECK (true);



CREATE POLICY "Service role full access scaffold_nodes" ON "public"."scaffold_nodes" TO "service_role" USING (true) WITH CHECK (true);
...
CREATE POLICY "Service role full access skill_fragments" ON "public"."skill_fragments" TO "service_role" USING (true) WITH CHECK (true);



CREATE POLICY "Service role full access skills" ON "public"."skills" TO "service_role" USING (true) WITH CHECK (true);
...
CREATE POLICY "User can read model home assignments via consent" ON "public"."model_home_assignments" FOR SELECT TO "authenticated" USING ((EXISTS ( SELECT 1
   FROM "public"."model_home_consent_grants" "g"
  WHERE (("g"."model_home_id" = "model_home_assignments"."model_home_id") AND ("g"."user_id" = ( SELECT "auth"."uid"() AS "uid"))))));



CREATE POLICY "User can read model home capabilities via consent" ON "public"."model_home_capabilities" FOR SELECT TO "authenticated" USING ((EXISTS ( SELECT 1
   FROM "public"."model_home_consent_grants" "g"
  WHERE (("g"."model_home_id" = "model_home_capabilities"."model_home_id") AND ("g"."user_id" = ( SELECT "auth"."uid"() AS "uid"))))));



CREATE POLICY "User can read model home evaluations via consent" ON "public"."model_home_evaluations" FOR SELECT TO "authenticated" USING ((EXISTS ( SELECT 1
   FROM "public"."model_home_consent_grants" "g"
  WHERE (("g"."model_home_id" = "model_home_evaluations"."model_home_id") AND ("g"."user_id" = ( SELECT "auth"."uid"() AS "uid"))))));



CREATE POLICY "User can read own consent grants" ON "public"."model_home_consent_grants" FOR SELECT TO "authenticated" USING (("user_id" = ( SELECT "auth"."uid"() AS "uid")));



CREATE POLICY "User can read own model home events" ON "public"."model_home_events" FOR SELECT TO "authenticated" USING (("user_id" = ( SELECT "auth"."uid"() AS "uid")));



CREATE POLICY "Users access own artifacts" ON "public"."inner_world_artifacts" USING (("auth"."uid"() = "user_id")) WITH CHECK (("auth"."uid"() = "user_id"));



CREATE POLICY "Users access own blueprints" ON "public"."blueprints" USING (("auth"."uid"() = "user_id")) WITH CHECK (("auth"."uid"() = "user_id"));



CREATE POLICY "Users access own insights" ON "public"."insights" USING (("auth"."uid"() = "user_id")) WITH CHECK (("auth"."uid"() = "user_id"));



CREATE POLICY "Users access own journals" ON "public"."journals" USING (("auth"."uid"() = "user_id")) WITH CHECK (("auth"."uid"() = "user_id"));



CREATE POLICY "Users access own preferences" ON "public"."user_preferences" USING (("auth"."uid"() = "user_id")) WITH CHECK (("auth"."uid"() = "user_id"));



CREATE POLICY "Users access own scrapbook" ON "public"."scrapbook_items" USING (("auth"."uid"() = "user_id")) WITH CHECK (("auth"."uid"() = "user_id"));



CREATE POLICY "Users can insert own analyses" ON "public"."musical_dna_analyses" FOR INSERT TO "authenticated" WITH CHECK ((( SELECT ("auth"."uid"())::"text" AS "uid") = "user_id"));



CREATE POLICY "Users can insert own bucket drops" ON "public"."bucket_drops" FOR INSERT TO "authenticated" WITH CHECK ((( SELECT ("auth"."uid"())::"text" AS "uid") = "user_id"));



CREATE POLICY "Users can insert own manual portrait inference runs" ON "public"."portrait_inference_runs" FOR INSERT WITH CHECK ((("auth"."uid"() = "user_id") AND ("triggered_by" = 'manual'::"text")));



CREATE POLICY "Users can insert own portrait render events" ON "public"."portrait_render_events" FOR INSERT WITH CHECK (("auth"."uid"() = "user_id"));



CREATE POLICY "Users can insert own sessions" ON "public"."billy_sessions" FOR INSERT TO "authenticated" WITH CHECK ((( SELECT ("auth"."uid"())::"text" AS "uid") = "user_id"));



CREATE POLICY "Users can insert own tribunal sessions" ON "public"."tribunal_sessions" FOR INSERT TO "authenticated" WITH CHECK ((( SELECT ("auth"."uid"())::"text" AS "uid") = "user_id"));
...
CREATE POLICY "Users can read own analyses" ON "public"."musical_dna_analyses" FOR SELECT TO "authenticated" USING ((( SELECT ("auth"."uid"())::"text" AS "uid") = "user_id"));



CREATE POLICY "Users can read own bucket drops" ON "public"."bucket_drops" FOR SELECT TO "authenticated" USING ((( SELECT ("auth"."uid"())::"text" AS "uid") = "user_id"));



CREATE POLICY "Users can read own portrait dimensions" ON "public"."portrait_dimensions" FOR SELECT USING (("auth"."uid"() = "user_id"));



CREATE POLICY "Users can read own portrait inference runs" ON "public"."portrait_inference_runs" FOR SELECT USING (("auth"."uid"() = "user_id"));



CREATE POLICY "Users can read own portrait render events" ON "public"."portrait_render_events" FOR SELECT USING (("auth"."uid"() = "user_id"));



CREATE POLICY "Users can read own portraits" ON "public"."profile_portraits" FOR SELECT USING (("auth"."uid"() = "user_id"));



CREATE POLICY "Users can read own profile" ON "public"."consciousness_profiles" FOR SELECT TO "authenticated" USING ((( SELECT ("auth"."uid"())::"text" AS "uid") = "user_id"));
...
CREATE POLICY "Users can read own row" ON "public"."app_users" FOR SELECT TO "authenticated" USING ((( SELECT ("auth"."uid"())::"text" AS "uid") = "id"));



CREATE POLICY "Users can read own sessions" ON "public"."billy_sessions" FOR SELECT TO "authenticated" USING ((( SELECT ("auth"."uid"())::"text" AS "uid") = "user_id"));



CREATE POLICY "Users can read own tribunal sessions" ON "public"."tribunal_sessions" FOR SELECT TO "authenticated" USING ((( SELECT ("auth"."uid"())::"text" AS "uid") = "user_id"));



CREATE POLICY "Users can update own bucket drops" ON "public"."bucket_drops" FOR UPDATE TO "authenticated" USING ((( SELECT ("auth"."uid"())::"text" AS "uid") = "user_id")) WITH CHECK ((( SELECT ("auth"."uid"())::"text" AS "uid") = "user_id"));



CREATE POLICY "Users can update own profile" ON "public"."consciousness_profiles" FOR UPDATE TO "authenticated" USING ((( SELECT ("auth"."uid"())::"text" AS "uid") = "user_id")) WITH CHECK ((( SELECT ("auth"."uid"())::"text" AS "uid") = "user_id"));



CREATE POLICY "Users can update own row" ON "public"."app_users" FOR UPDATE TO "authenticated" USING ((( SELECT ("auth"."uid"())::"text" AS "uid") = "id")) WITH CHECK ((( SELECT ("auth"."uid"())::"text" AS "uid") = "id"));



CREATE POLICY "Users can update own sessions" ON "public"."billy_sessions" FOR UPDATE TO "authenticated" USING ((( SELECT ("auth"."uid"())::"text" AS "uid") = "user_id")) WITH CHECK ((( SELECT ("auth"."uid"())::"text" AS "uid") = "user_id"));



CREATE POLICY "Users can update own tribunal sessions" ON "public"."tribunal_sessions" FOR UPDATE TO "authenticated" USING ((( SELECT ("auth"."uid"())::"text" AS "uid") = "user_id")) WITH CHECK ((( SELECT ("auth"."uid"())::"text" AS "uid") = "user_id"));



CREATE POLICY "Users can upsert own profile" ON "public"."consciousness_profiles" FOR INSERT TO "authenticated" WITH CHECK ((( SELECT ("auth"."uid"())::"text" AS "uid") = "user_id"));



CREATE POLICY "Users can view own portrait queue status" ON "public"."portrait_inference_queue" FOR SELECT USING (("auth"."uid"() = "user_id"));



CREATE POLICY "Users manage own artifacts" ON "public"."_deprecated_artifacts" USING (("owner_user_id" = "auth"."uid"())) WITH CHECK (("owner_user_id" = "auth"."uid"()));



CREATE POLICY "Users manage own capture_events" ON "public"."capture_events" USING (("owner_user_id" = "auth"."uid"())) WITH CHECK (("owner_user_id" = "auth"."uid"()));



CREATE POLICY "Users manage own profile_pipeline_runs" ON "public"."profile_pipeline_runs" USING (("owner_user_id" = "auth"."uid"())) WITH CHECK (("owner_user_id" = "auth"."uid"()));



CREATE POLICY "Users manage own scaffold_nodes" ON "public"."scaffold_nodes" USING (("owner_user_id" = "auth"."uid"())) WITH CHECK (("owner_user_id" = "auth"."uid"()));
...
CREATE POLICY "Users manage their own files" ON "public"."user_files" USING (("user_id" = "auth"."uid"())) WITH CHECK (("user_id" = "auth"."uid"()));
...
CREATE POLICY "Users manage their own inner world artifacts" ON "public"."inner_world_artifacts" USING (("user_id" = "auth"."uid"())) WITH CHECK (("user_id" = "auth"."uid"()));
...
CREATE POLICY "Users read own identity_claims" ON "public"."identity_claims" FOR SELECT USING (("owner_user_id" = "auth"."uid"()));



CREATE POLICY "Users read own profile_pipeline_run_links" ON "public"."profile_pipeline_run_links" FOR SELECT USING ((EXISTS ( SELECT 1
   FROM "public"."profile_pipeline_runs" "r"
  WHERE (("r"."run_id" = "profile_pipeline_run_links"."run_id") AND ("r"."owner_user_id" = "auth"."uid"())))));
...
CREATE POLICY "Users submit own identity_claims" ON "public"."identity_claims" FOR INSERT WITH CHECK (("owner_user_id" = "auth"."uid"()));
...
CREATE POLICY "ac_select_auth" ON "public"."annotation_concepts" FOR SELECT TO "authenticated" USING (true);
...
CREATE POLICY "agents_delete_own" ON "public"."agents" FOR DELETE USING (("owner_user_id" = "auth"."uid"()));



CREATE POLICY "agents_insert_own" ON "public"."agents" FOR INSERT WITH CHECK (("owner_user_id" = "auth"."uid"()));



CREATE POLICY "agents_select_own" ON "public"."agents" FOR SELECT USING ((("owner_user_id" = "auth"."uid"()) OR ("owner_user_id" IS NULL)));



CREATE POLICY "agents_update_own" ON "public"."agents" FOR UPDATE USING (("owner_user_id" = "auth"."uid"())) WITH CHECK (("owner_user_id" = "auth"."uid"()));
...
CREATE POLICY "app_users_insert_own" ON "public"."app_users" FOR INSERT WITH CHECK (("id" = ("auth"."uid"())::"text"));



CREATE POLICY "app_users_select_own" ON "public"."app_users" FOR SELECT USING (("id" = ("auth"."uid"())::"text"));
...
CREATE POLICY "approvals_insert_approver" ON "public"."approvals" FOR INSERT WITH CHECK (("approver_user_id" = "auth"."uid"()));



CREATE POLICY "approvals_select_participant" ON "public"."approvals" FOR SELECT USING (("approver_user_id" = "auth"."uid"()));
...
CREATE POLICY "authenticated manage embodiment_readiness_scores" ON "public"."embodiment_readiness_scores" TO "authenticated" USING ((("recorded_by" = "auth"."uid"()) OR "public"."is_founder_admin_user"("auth"."uid"()))) WITH CHECK ((("recorded_by" = "auth"."uid"()) OR "public"."is_founder_admin_user"("auth"."uid"())));
...
CREATE POLICY "authenticated manage own context_injection_packets" ON "public"."context_injection_packets" TO "authenticated" USING (("auth_user_id" = "auth"."uid"())) WITH CHECK (("auth_user_id" = "auth"."uid"()));



CREATE POLICY "authenticated manage own context_injection_rules" ON "public"."context_injection_rules" TO "authenticated" USING (("auth_user_id" = "auth"."uid"())) WITH CHECK (("auth_user_id" = "auth"."uid"()));
...
CREATE POLICY "authenticated manage own human_cognition_profiles" ON "public"."human_cognition_profiles" TO "authenticated" USING (("auth_user_id" = "auth"."uid"())) WITH CHECK (("auth_user_id" = "auth"."uid"()));



CREATE POLICY "authenticated manage own human_consciousness_profiles" ON "public"."human_consciousness_profiles" TO "authenticated" USING (("auth_user_id" = "auth"."uid"())) WITH CHECK (("auth_user_id" = "auth"."uid"()));



CREATE POLICY "authenticated manage own human_context_views" ON "public"."human_context_views" TO "authenticated" USING (("auth_user_id" = "auth"."uid"())) WITH CHECK (("auth_user_id" = "auth"."uid"()));



CREATE POLICY "authenticated manage own human_continuity_snapshots" ON "public"."human_continuity_snapshots" TO "authenticated" USING (("auth_user_id" = "auth"."uid"())) WITH CHECK (("auth_user_id" = "auth"."uid"()));



CREATE POLICY "authenticated manage own human_identity_evidence" ON "public"."human_identity_evidence" TO "authenticated" USING (("auth_user_id" = "auth"."uid"())) WITH CHECK (("auth_user_id" = "auth"."uid"()));



CREATE POLICY "authenticated manage own human_identity_mutations" ON "public"."human_identity_mutations" TO "authenticated" USING (("auth_user_id" = "auth"."uid"())) WITH CHECK (("auth_user_id" = "auth"."uid"()));



CREATE POLICY "authenticated manage own human_identity_profiles" ON "public"."human_identity_profiles" TO "authenticated" USING (("auth_user_id" = "auth"."uid"())) WITH CHECK (("auth_user_id" = "auth"."uid"()));



CREATE POLICY "authenticated manage own human_identity_review_events" ON "public"."human_identity_review_events" TO "authenticated" USING (("auth_user_id" = "auth"."uid"())) WITH CHECK (("auth_user_id" = "auth"."uid"()));



CREATE POLICY "authenticated manage own human_identity_rollback_events" ON "public"."human_identity_rollback_events" TO "authenticated" USING (("auth_user_id" = "auth"."uid"())) WITH CHECK (("auth_user_id" = "auth"."uid"()));



CREATE POLICY "authenticated manage own human_memory_records" ON "public"."human_memory_records" TO "authenticated" USING (("auth_user_id" = "auth"."uid"())) WITH CHECK (("auth_user_id" = "auth"."uid"()));



CREATE POLICY "authenticated manage own human_personality_profiles" ON "public"."human_personality_profiles" TO "authenticated" USING (("auth_user_id" = "auth"."uid"())) WITH CHECK (("auth_user_id" = "auth"."uid"()));



CREATE POLICY "authenticated manage own human_relationship_edges" ON "public"."human_relationship_edges" TO "authenticated" USING (("auth_user_id" = "auth"."uid"())) WITH CHECK (("auth_user_id" = "auth"."uid"()));



CREATE POLICY "authenticated manage own identity_subjects" ON "public"."identity_subjects" TO "authenticated" USING ((("auth_user_id" = "auth"."uid"()) OR ("app_user_id" = ("auth"."uid"())::"text"))) WITH CHECK ((("auth_user_id" = "auth"."uid"()) OR ("app_user_id" = ("auth"."uid"())::"text")));
...
CREATE POLICY "authenticated read own gate_orders" ON "public"."gate_orders" FOR SELECT TO "authenticated" USING (("customer_email" = ( SELECT "users"."email"
   FROM "public"."users"
  WHERE ("users"."id" = "auth"."uid"()))));



CREATE POLICY "authenticated read own gate_package_drafts" ON "public"."gate_package_drafts" FOR SELECT TO "authenticated" USING (("buyer_email" = ( SELECT "users"."email"
   FROM "public"."users"
  WHERE ("users"."id" = "auth"."uid"()))));
...
CREATE POLICY "authenticated review own embodiment_mutation_proposals" ON "public"."embodiment_mutation_proposals" FOR SELECT TO "authenticated" USING ((("submitted_by" = "auth"."uid"()) OR "public"."is_founder_admin_user"("auth"."uid"())));



CREATE POLICY "authenticated submit embodiment_mutation_proposals" ON "public"."embodiment_mutation_proposals" FOR INSERT TO "authenticated" WITH CHECK ((("submitted_by" = "auth"."uid"()) OR ("submitted_by" IS NULL)));
...
CREATE POLICY "av_insert_via_agent" ON "public"."agent_versions" FOR INSERT WITH CHECK ((EXISTS ( SELECT 1
   FROM "public"."agents" "a"
  WHERE (("a"."agent_id" = "agent_versions"."agent_id") AND ("a"."owner_user_id" = "auth"."uid"())))));



CREATE POLICY "av_select_via_agent" ON "public"."agent_versions" FOR SELECT USING ((EXISTS ( SELECT 1
   FROM "public"."agents" "a"
  WHERE (("a"."agent_id" = "agent_versions"."agent_id") AND (("a"."owner_user_id" = "auth"."uid"()) OR ("a"."owner_user_id" IS NULL))))));



CREATE POLICY "av_update_via_agent" ON "public"."agent_versions" FOR UPDATE USING ((EXISTS ( SELECT 1
   FROM "public"."agents" "a"
  WHERE (("a"."agent_id" = "agent_versions"."agent_id") AND ("a"."owner_user_id" = "auth"."uid"())))));
...
CREATE POLICY "billy_sessions_insert_own" ON "public"."billy_sessions" FOR INSERT WITH CHECK (("user_id" = ("auth"."uid"())::"text"));



CREATE POLICY "billy_sessions_select_own" ON "public"."billy_sessions" FOR SELECT USING (("user_id" = ("auth"."uid"())::"text"));
...
CREATE POLICY "bucket_drops_delete_own" ON "public"."bucket_drops" FOR DELETE USING (("user_id" = ("auth"."uid"())::"text"));



CREATE POLICY "bucket_drops_insert_own" ON "public"."bucket_drops" FOR INSERT WITH CHECK (("user_id" = ("auth"."uid"())::"text"));



CREATE POLICY "bucket_drops_select_own" ON "public"."bucket_drops" FOR SELECT USING (("user_id" = ("auth"."uid"())::"text"));
...
CREATE POLICY "collaborators insert" ON "public"."collaborators" FOR INSERT TO "authenticated" WITH CHECK (true);
...
CREATE POLICY "concepts_select_auth" ON "public"."concepts" FOR SELECT TO "authenticated" USING (true);
...
CREATE POLICY "cp_delete_own" ON "public"."collaborator_permissions" FOR DELETE TO "authenticated" USING (("collaborator_id" = "auth"."uid"()));



CREATE POLICY "cp_insert_own" ON "public"."collaborator_permissions" FOR INSERT TO "authenticated" WITH CHECK (("collaborator_id" = "auth"."uid"()));



CREATE POLICY "cp_insert_own" ON "public"."consciousness_profiles" FOR INSERT WITH CHECK (("user_id" = ("auth"."uid"())::"text"));



CREATE POLICY "cp_select_own" ON "public"."collaborator_permissions" FOR SELECT TO "authenticated" USING (("collaborator_id" = "auth"."uid"()));



CREATE POLICY "cp_select_own" ON "public"."consciousness_profiles" FOR SELECT USING (("user_id" = ("auth"."uid"())::"text"));



CREATE POLICY "cp_update_own" ON "public"."collaborator_permissions" FOR UPDATE TO "authenticated" USING (("collaborator_id" = "auth"."uid"())) WITH CHECK (("collaborator_id" = "auth"."uid"()));



CREATE POLICY "cp_update_own" ON "public"."consciousness_profiles" FOR UPDATE USING (("user_id" = ("auth"."uid"())::"text")) WITH CHECK (("user_id" = ("auth"."uid"())::"text"));



CREATE POLICY "cr_select_if_user_owns_source_or_target" ON "public"."collaborator_relationships" FOR SELECT TO "authenticated" USING (((EXISTS ( SELECT 1
   FROM "public"."collaborators" "c"
  WHERE (("c"."collaborator_id" = "collaborator_relationships"."source_collaborator_id") AND ("c"."auth_user_id" = "auth"."uid"())))) OR (EXISTS ( SELECT 1
   FROM "public"."collaborators" "c"
  WHERE (("c"."collaborator_id" = "collaborator_relationships"."target_collaborator_id") AND ("c"."auth_user_id" = "auth"."uid"()))))));
...
CREATE POLICY "da_select_via_agent" ON "public"."deployment_artifacts" FOR SELECT USING ((EXISTS ( SELECT 1
   FROM ("public"."agent_versions" "av"
     JOIN "public"."agents" "a" ON (("a"."agent_id" = "av"."agent_id")))
  WHERE (("av"."version_id" = "deployment_artifacts"."version_id") AND (("a"."owner_user_id" = "auth"."uid"()) OR ("a"."owner_user_id" IS NULL))))));



CREATE POLICY "dc_select_auth" ON "public"."document_concepts" FOR SELECT TO "authenticated" USING (true);
...
CREATE POLICY "di_memory_service_write" ON "public"."di_memory_events" FOR INSERT WITH CHECK (("auth"."role"() = 'service_role'::"text"));



CREATE POLICY "di_memory_user_read" ON "public"."di_memory_events" FOR SELECT USING (("auth"."uid"() = "user_id"));
...
CREATE POLICY "di_sessions_user_own" ON "public"."di_sessions" USING (("auth"."uid"() = "user_id")) WITH CHECK (("auth"."uid"() = "user_id"));
...
CREATE POLICY "emb_select_auth" ON "public"."embeddings" FOR SELECT TO "authenticated" USING (true);
...
CREATE POLICY "er_select_auth" ON "public"."eval_rubrics" FOR SELECT TO "authenticated" USING (true);



CREATE POLICY "er_select_via_run" ON "public"."eval_results" FOR SELECT USING ((EXISTS ( SELECT 1
   FROM "public"."training_runs" "r"
  WHERE (("r"."run_id" = "eval_results"."run_id") AND (("r"."requested_by" = "auth"."uid"()) OR ("r"."approver_user_id" = "auth"."uid"()))))));
...
CREATE POLICY "founder review embodiment_mutation_proposals" ON "public"."embodiment_mutation_proposals" FOR UPDATE TO "authenticated" USING ("public"."is_founder_admin_user"("auth"."uid"())) WITH CHECK ("public"."is_founder_admin_user"("auth"."uid"()));



CREATE POLICY "founder review embodiment_review_log" ON "public"."embodiment_review_log" TO "authenticated" USING ("public"."is_founder_admin_user"("auth"."uid"())) WITH CHECK ("public"."is_founder_admin_user"("auth"."uid"()));
...
CREATE POLICY "gate_buyers_insert_anon" ON "public"."gate_buyers" FOR INSERT TO "authenticated", "anon" WITH CHECK (true);
...
CREATE POLICY "gate_pd_insert_anon" ON "public"."gate_package_drafts" FOR INSERT TO "authenticated", "anon" WITH CHECK (true);



CREATE POLICY "gate_pd_update_anon" ON "public"."gate_package_drafts" FOR UPDATE TO "authenticated", "anon" USING (true) WITH CHECK (true);



CREATE POLICY "gate_sr_insert_anon" ON "public"."gate_support_requests" FOR INSERT TO "authenticated", "anon" WITH CHECK (true);
...
CREATE POLICY "insert own onboarding events" ON "public"."collaborator_onboarding_events" FOR INSERT TO "authenticated" WITH CHECK (("collaborator_id" = "auth"."uid"()));
...
CREATE POLICY "kf_select_auth" ON "public"."knowledge_fragments" FOR SELECT TO "authenticated" USING (true);
...
ALTER TABLE "public"."kv_store_441770b7" ENABLE ROW LEVEL SECURITY;


CREATE POLICY "la_select_auth" ON "public"."loom_annotations" FOR SELECT TO "authenticated" USING (true);
...
CREATE POLICY "m_select_auth" ON "public"."models" FOR SELECT TO "authenticated" USING (("enabled" = true));
...
CREATE POLICY "mda_insert_own" ON "public"."musical_dna_analyses" FOR INSERT WITH CHECK (("user_id" = ("auth"."uid"())::"text"));



CREATE POLICY "mda_select_own" ON "public"."musical_dna_analyses" FOR SELECT USING (("user_id" = ("auth"."uid"())::"text"));
...
CREATE POLICY "mp_select_auth" ON "public"."model_providers" FOR SELECT TO "authenticated" USING (("enabled" = true));
...
CREATE POLICY "pr_select_auth" ON "public"."processing_runs" FOR SELECT TO "authenticated" USING (true);
...
CREATE POLICY "read_own_rows" ON "public"."collaborator_embodiment_links" FOR SELECT TO "authenticated" USING (("collaborator_id" = "auth"."uid"()));
...
CREATE POLICY "sc_select_auth" ON "public"."scenarios" FOR SELECT TO "authenticated" USING (true);
...
CREATE POLICY "select own onboarding events" ON "public"."collaborator_onboarding_events" FOR SELECT TO "authenticated" USING (("collaborator_id" = "auth"."uid"()));
...
CREATE POLICY "service_role full access" ON "public"."app_users" TO "service_role" USING (true) WITH CHECK (true);



CREATE POLICY "service_role full access" ON "public"."billy_sessions" TO "service_role" USING (true) WITH CHECK (true);



CREATE POLICY "service_role full access" ON "public"."bucket_drops" TO "service_role" USING (true) WITH CHECK (true);



CREATE POLICY "service_role full access" ON "public"."consciousness_profiles" TO "service_role" USING (true) WITH CHECK (true);



CREATE POLICY "service_role full access" ON "public"."musical_dna_analyses" TO "service_role" USING (true) WITH CHECK (true);



CREATE POLICY "service_role full access" ON "public"."processing_runs" TO "service_role" USING (true) WITH CHECK (true);



CREATE POLICY "service_role full access" ON "public"."tribunal_sessions" TO "service_role" USING (true) WITH CHECK (true);
...
CREATE POLICY "service_role full access gate_artifacts" ON "public"."gate_artifacts" TO "service_role" USING (true) WITH CHECK (true);



CREATE POLICY "service_role full access gate_build_jobs" ON "public"."gate_build_jobs" TO "service_role" USING (true) WITH CHECK (true);



CREATE POLICY "service_role full access gate_buyers" ON "public"."gate_buyers" TO "service_role" USING (true) WITH CHECK (true);



CREATE POLICY "service_role full access gate_order_items" ON "public"."gate_order_items" TO "service_role" USING (true) WITH CHECK (true);



CREATE POLICY "service_role full access gate_orders" ON "public"."gate_orders" TO "service_role" USING (true) WITH CHECK (true);



CREATE POLICY "service_role full access gate_package_drafts" ON "public"."gate_package_drafts" TO "service_role" USING (true) WITH CHECK (true);



CREATE POLICY "service_role full access gate_support_requests" ON "public"."gate_support_requests" TO "service_role" USING (true) WITH CHECK (true);
...
CREATE POLICY "sf_select_auth" ON "public"."skill_fragments" FOR SELECT TO "authenticated" USING (true);
...
CREATE POLICY "ss_insert_own" ON "public"."scenario_sets" FOR INSERT WITH CHECK (("created_by" = "auth"."uid"()));



CREATE POLICY "ss_select_auth" ON "public"."scenario_sets" FOR SELECT TO "authenticated" USING (true);



CREATE POLICY "sum_select_auth" ON "public"."summaries" FOR SELECT TO "authenticated" USING (true);
...
CREATE POLICY "te_select_auth" ON "public"."tribunal_events" FOR SELECT TO "authenticated" USING (true);



CREATE POLICY "tev_select_auth" ON "public"."tribunal_evidence" FOR SELECT TO "authenticated" USING (true);



CREATE POLICY "tj_select_via_run" ON "public"."trainer_jobs" FOR SELECT USING ((EXISTS ( SELECT 1
   FROM "public"."training_runs" "r"
  WHERE (("r"."run_id" = "trainer_jobs"."run_id") AND (("r"."requested_by" = "auth"."uid"()) OR ("r"."approver_user_id" = "auth"."uid"()))))));



CREATE POLICY "tr_insert_own" ON "public"."training_runs" FOR INSERT WITH CHECK (("requested_by" = "auth"."uid"()));



CREATE POLICY "tr_select_participant" ON "public"."training_runs" FOR SELECT USING ((("requested_by" = "auth"."uid"()) OR ("approver_user_id" = "auth"."uid"())));



CREATE POLICY "tr_update_participant" ON "public"."training_runs" FOR UPDATE USING ((("requested_by" = "auth"."uid"()) OR ("approver_user_id" = "auth"."uid"())));
...
CREATE POLICY "ts_insert_own" ON "public"."tribunal_sessions" FOR INSERT WITH CHECK (("user_id" = ("auth"."uid"())::"text"));



CREATE POLICY "ts_select_own" ON "public"."tribunal_sessions" FOR SELECT USING (("user_id" = ("auth"."uid"())::"text"));



CREATE POLICY "tstep_select_via_run" ON "public"."training_steps" FOR SELECT USING ((EXISTS ( SELECT 1
   FROM "public"."training_runs" "r"
  WHERE (("r"."run_id" = "training_steps"."run_id") AND (("r"."requested_by" = "auth"."uid"()) OR ("r"."approver_user_id" = "auth"."uid"()))))));
...
ALTER PUBLICATION "supabase_realtime" ADD TABLE ONLY "public"."skill_fragments";
...
GRANT ALL ON FUNCTION "public"."before_user_created_hook"("event" "jsonb") TO "authenticated";
GRANT ALL ON FUNCTION "public"."before_user_created_hook"("event" "jsonb") TO "anon";
...
REVOKE ALL ON FUNCTION "public"."fn_corpus_harvest_on_pipeline_complete"() FROM PUBLIC;
GRANT ALL ON FUNCTION "public"."fn_corpus_harvest_on_pipeline_complete"() TO "service_role";
...
GRANT ALL ON FUNCTION "public"."gv_touch_updated_at"() TO "anon";
GRANT ALL ON FUNCTION "public"."gv_touch_updated_at"() TO "authenticated";
GRANT ALL ON FUNCTION "public"."gv_touch_updated_at"() TO "service_role";
...
REVOKE ALL ON FUNCTION "public"."has_valid_subscription_access"("p_user_id" "uuid") FROM PUBLIC;
...
REVOKE ALL ON FUNCTION "public"."promote_drop_to_memory"("p_drop_id" "uuid", "p_auth_user_id" "uuid", "p_kind" "text", "p_scope" "text") FROM PUBLIC;
GRANT ALL ON FUNCTION "public"."promote_drop_to_memory"("p_drop_id" "uuid", "p_auth_user_id" "uuid", "p_kind" "text", "p_scope" "text") TO "service_role";
...
GRANT ALL ON FUNCTION "public"."searchSkills"("query_text" "text", "match_count" integer, "filter_skill_name" "text", "filter_category" "text", "filter_tags" "text"[]) TO "anon";
GRANT ALL ON FUNCTION "public"."searchSkills"("query_text" "text", "match_count" integer, "filter_skill_name" "text", "filter_category" "text", "filter_tags" "text"[]) TO "authenticated";
GRANT ALL ON FUNCTION "public"."searchSkills"("query_text" "text", "match_count" integer, "filter_skill_name" "text", "filter_category" "text", "filter_tags" "text"[]) TO "service_role";
...
GRANT ALL ON FUNCTION "public"."search_skill_fragments"("query_text" "text", "match_count" integer, "filter_skill" "text") TO "anon";
GRANT ALL ON FUNCTION "public"."search_skill_fragments"("query_text" "text", "match_count" integer, "filter_skill" "text") TO "authenticated";
GRANT ALL ON FUNCTION "public"."search_skill_fragments"("query_text" "text", "match_count" integer, "filter_skill" "text") TO "service_role";



GRANT ALL ON FUNCTION "public"."search_skill_fragments"("search_query" "text", "match_count" integer, "filter_skill_name" "text", "filter_tags" "text"[]) TO "authenticated";
GRANT ALL ON FUNCTION "public"."search_skill_fragments"("search_query" "text", "match_count" integer, "filter_skill_name" "text", "filter_tags" "text"[]) TO "service_role";



GRANT ALL ON FUNCTION "public"."search_skills"("query_text" "text", "match_count" integer, "filter_skill_name" "text", "filter_category" "text", "filter_tags" "text"[]) TO "anon";
GRANT ALL ON FUNCTION "public"."search_skills"("query_text" "text", "match_count" integer, "filter_skill_name" "text", "filter_category" "text", "filter_tags" "text"[]) TO "authenticated";
GRANT ALL ON FUNCTION "public"."search_skills"("query_text" "text", "match_count" integer, "filter_skill_name" "text", "filter_category" "text", "filter_tags" "text"[]) TO "service_role";
...
GRANT ALL ON TABLE "public"."_deprecated_build_jobs" TO "anon";
GRANT ALL ON TABLE "public"."_deprecated_build_jobs" TO "authenticated";
GRANT ALL ON TABLE "public"."_deprecated_build_jobs" TO "service_role";



GRANT ALL ON TABLE "public"."_deprecated_buyers" TO "anon";
GRANT ALL ON TABLE "public"."_deprecated_buyers" TO "authenticated";
GRANT ALL ON TABLE "public"."_deprecated_buyers" TO "service_role";



GRANT ALL ON TABLE "public"."_deprecated_order_items" TO "anon";
GRANT ALL ON TABLE "public"."_deprecated_order_items" TO "authenticated";
GRANT ALL ON TABLE "public"."_deprecated_order_items" TO "service_role";
...
GRANT ALL ON TABLE "public"."_deprecated_package_drafts" TO "anon";
GRANT ALL ON TABLE "public"."_deprecated_package_drafts" TO "authenticated";
GRANT ALL ON TABLE "public"."_deprecated_package_drafts" TO "service_role";



GRANT ALL ON TABLE "public"."_deprecated_support_requests" TO "anon";
GRANT ALL ON TABLE "public"."_deprecated_support_requests" TO "authenticated";
GRANT ALL ON TABLE "public"."_deprecated_support_requests" TO "service_role";
...
GRANT INSERT,REFERENCES,DELETE,TRIGGER,TRUNCATE,MAINTAIN,UPDATE ON TABLE "public"."active_agent_manifests" TO "authenticated";
...
GRANT INSERT,REFERENCES,DELETE,TRIGGER,TRUNCATE,MAINTAIN,UPDATE ON TABLE "public"."approved_library_assets_by_agent" TO "authenticated";
...
GRANT ALL ON TABLE "public"."corpus_harvest_events" TO "anon";
GRANT ALL ON TABLE "public"."corpus_harvest_events" TO "authenticated";
...
GRANT ALL ON TABLE "public"."gate_artifacts" TO "anon";
GRANT ALL ON TABLE "public"."gate_artifacts" TO "authenticated";
GRANT ALL ON TABLE "public"."gate_artifacts" TO "service_role";



GRANT ALL ON TABLE "public"."gate_build_jobs" TO "anon";
GRANT ALL ON TABLE "public"."gate_build_jobs" TO "authenticated";
GRANT ALL ON TABLE "public"."gate_build_jobs" TO "service_role";



GRANT ALL ON TABLE "public"."gate_buyers" TO "anon";
GRANT ALL ON TABLE "public"."gate_buyers" TO "authenticated";
GRANT ALL ON TABLE "public"."gate_buyers" TO "service_role";



GRANT ALL ON TABLE "public"."gate_order_items" TO "anon";
GRANT ALL ON TABLE "public"."gate_order_items" TO "authenticated";
GRANT ALL ON TABLE "public"."gate_order_items" TO "service_role";



GRANT ALL ON TABLE "public"."gate_orders" TO "anon";
GRANT ALL ON TABLE "public"."gate_orders" TO "authenticated";
GRANT ALL ON TABLE "public"."gate_orders" TO "service_role";



GRANT ALL ON TABLE "public"."gate_package_drafts" TO "anon";
GRANT ALL ON TABLE "public"."gate_package_drafts" TO "authenticated";
GRANT ALL ON TABLE "public"."gate_package_drafts" TO "service_role";



GRANT ALL ON TABLE "public"."gate_support_requests" TO "anon";
GRANT ALL ON TABLE "public"."gate_support_requests" TO "authenticated";
...
GRANT INSERT,REFERENCES,DELETE,TRIGGER,TRUNCATE,MAINTAIN,UPDATE ON TABLE "public"."knowledge_stats" TO "anon";
GRANT INSERT,REFERENCES,DELETE,TRIGGER,TRUNCATE,MAINTAIN,UPDATE ON TABLE "public"."knowledge_stats" TO "authenticated";
...
GRANT ALL ON TABLE "public"."kv_store_441770b7" TO "anon";
GRANT ALL ON TABLE "public"."kv_store_441770b7" TO "authenticated";
GRANT ALL ON TABLE "public"."kv_store_441770b7" TO "service_role";
...
GRANT INSERT,REFERENCES,DELETE,TRIGGER,TRUNCATE,MAINTAIN,UPDATE ON TABLE "public"."manifest_file_pull" TO "anon";
GRANT INSERT,REFERENCES,DELETE,TRIGGER,TRUNCATE,MAINTAIN,UPDATE ON TABLE "public"."manifest_file_pull" TO "authenticated";
...
GRANT INSERT,REFERENCES,DELETE,TRIGGER,TRUNCATE,MAINTAIN,UPDATE ON TABLE "public"."pending_embodiment_mutations" TO "anon";
GRANT INSERT,REFERENCES,DELETE,TRIGGER,TRUNCATE,MAINTAIN,UPDATE ON TABLE "public"."pending_embodiment_mutations" TO "authenticated";
...
GRANT ALL ON SEQUENCE "public"."signup_allowlist_id_seq" TO "anon";
GRANT ALL ON SEQUENCE "public"."signup_allowlist_id_seq" TO "authenticated";
GRANT ALL ON SEQUENCE "public"."signup_allowlist_id_seq" TO "service_role";
...
GRANT INSERT,REFERENCES,DELETE,TRIGGER,TRUNCATE,MAINTAIN,UPDATE ON TABLE "public"."skill_stats" TO "anon";
GRANT INSERT,REFERENCES,DELETE,TRIGGER,TRUNCATE,MAINTAIN,UPDATE ON TABLE "public"."skill_stats" TO "authenticated";
...
GRANT INSERT,REFERENCES,DELETE,TRIGGER,TRUNCATE,MAINTAIN,UPDATE ON TABLE "public"."trainer_memory_surfaces" TO "anon";
GRANT INSERT,REFERENCES,DELETE,TRIGGER,TRUNCATE,MAINTAIN,UPDATE ON TABLE "public"."trainer_memory_surfaces" TO "authenticated";
...
GRANT INSERT,REFERENCES,DELETE,TRIGGER,TRUNCATE,MAINTAIN,UPDATE ON TABLE "public"."trainer_queue_health_v" TO "anon";
GRANT INSERT,REFERENCES,DELETE,TRIGGER,TRUNCATE,MAINTAIN,UPDATE ON TABLE "public"."trainer_queue_health_v" TO "authenticated";
