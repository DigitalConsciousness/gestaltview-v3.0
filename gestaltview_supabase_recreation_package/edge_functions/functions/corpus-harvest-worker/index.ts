  ...
  for (const event of events) {
    try {
      ...
      const docContent = [
        ...
        payload.run_kind ? `[sector: ${payload.sector_adaptation ?? "general"}]` : "",
      ].filter(Boolean).join("\n\n");
      ...
      const { data: doc, error: docErr } = await supabase
        ...
        .insert({
          ...
          provenance:        { source: "corpus_harvest", harvest_event_id: event.id },
        })
        .select("document_id")
        .single();
      ...
      const embedRes = await fetch("https://api.openai.com/v1/embeddings", {
        ...
        body: JSON.stringify({ model: EMBED_MODEL, input: docContent }),
      });
