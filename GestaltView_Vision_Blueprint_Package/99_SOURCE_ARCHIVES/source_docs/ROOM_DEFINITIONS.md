...
The Blackboard Room contains a `SessionRecapGenerator` trigger. When fired, it reads the current session's captures and conversation history, calls the LLM via `routeLlm` (mode: "billy"), and produces a self-contained interactive HTML artifact — the session's living recap. That artifact is then routed to the Dynamic Inner World as a first-class museum object.
...
- `sessionRecap.ts` has malformed interface syntax (missing closing `}` on `RecapCapture` and `RecapMessage`) — fix before deployment  
- `SessionRecapGenerator.tsx` still calls Anthropic directly client-side — needs to be updated to call `/api/sessionRecap` and read `result.html` from the new response shape
