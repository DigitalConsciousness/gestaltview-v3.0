...
export type CouncilLlmCaller = (job: IsolatedCouncilJob) => Promise<LlmResult | string>;
...
function normalizeLlmResponse(result: Awaited<ReturnType<CouncilLlmCaller>>): string {
  return typeof result === "string" ? result : result.response;
}

function estimateResonance(result: Awaited<ReturnType<CouncilLlmCaller>>): number | undefined {
...
function tripsFallbackGuard(response: string, fallbackGuard: string): boolean {
  return response.trim().startsWith(fallbackGuard) || response.includes(fallbackGuard);
}
...
async function defaultCouncilLlmCaller(job: IsolatedCouncilJob): Promise<LlmResult> {
...
export async function runCouncil(
  ...
  const staggerMs = options.dispatchStaggerMs ?? COUNCIL_DISPATCH_STAGGER_MS;
  ...
  for (let i = 0; i < activeJobs.length; i++) {
    ...
    // Stagger all dispatches after the first.
    if (i > 0 && staggerMs > 0) {
    ...
    const primaryFallback = tripsFallbackGuard(primaryResponse, job.fallbackGuard);
