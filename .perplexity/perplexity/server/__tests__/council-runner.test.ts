...
import { buildIsolatedCouncilPrompt, COUNCIL_FALLBACK_GUARD } from "../../shared/embodiment/chat";
...
describe("council bulkhead runner", () => {
  it("builds isolated council prompts with the council contract and fallback guard", () => {
    ...
    expect(job.systemPrompt).toContain("Keep this voice separate from every other profile in the room.");
    expect(job.systemPrompt).toContain("Do not synthesize for the entire council unless explicitly asked to do so.");
    expect(job.systemPrompt).toContain("- Conversation mode: council");
    expect(job.systemPrompt).toContain("Corpus context only.");
  });

  it("marks current heartbeat-free generated profiles as stubs before dispatch", () => {
  ...
  it("builds assembly input from baked responses only", async () => {
    const result = await runCouncil("Synthesize", ["fallback-lane", "baked-lane"], [], {
