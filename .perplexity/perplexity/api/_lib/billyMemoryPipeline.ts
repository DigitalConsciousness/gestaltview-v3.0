...
import { getFounderContext, insertRow } from "./supabase.js";
import { listMemoryEntries } from "./supabase.js";
...
async function getBillyAgent(): Promise<AgentRow | null> {
  const rows = await selectRows<AgentRow>(
    "agents",
    buildQuery([
      ["slug", "eq.billy"],
...
export async function buildBillySessionSystemPrompt(params: {
  ...
  if (!hasSupabaseConfig() || !params.userId || params.userId === "guest-user") {
    return {
  ...
  const [profile, constitution, memoryRecords, agentMemories, userMemories, founderContext, autobiography, identitySubject] =
    await Promise.all([
      getEmbodimentProfile("billy"),
      getAgentConstitution(agent.agent_id),
      getAgentMemoryRecords(agent.agent_id),
      getAgentMemories(agent.agent_id),
      listMemoryEntries({
        userId: params.userId,
        limit: 8,
      }),
      getFounderContext(params.userId),
      getAgentAutobiography(agent.agent_id),
      getIdentitySubject(params.userId),
    ]);

  const memoryCount = memoryRecords.length + agentMemories.length + userMemories.length;
  ...
  const packetContent = composeIdentityContextBlock({
    ...
    constitution,
    memoryRecords,
    summaryMemories: agentMemories,
    persistentMemories: [...(userMemories as MemoryEntryRow[])],
    founderContext,
    autobiography,
  });
