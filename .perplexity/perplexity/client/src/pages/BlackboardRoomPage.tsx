...
const BBR_STYLES = `
  ...
  .bbr-typing-dot {
    ...
    animation: bbr-bounce 1.3s ease infinite;
  }
  .bbr-typing-dot:nth-child(2) { animation-delay: 0.18s; }
  .bbr-typing-dot:nth-child(3) { animation-delay: 0.36s; }
    ...
    : sessionEnded
      ...
      : isTimedOut
        ...
        : isRoundtable
          ? "Roundtable"
          : "Live session";
  const selectorCountLabel =
    isRoundtable && selectedPersonas.length > 1 ? `${selectedPersonas.length} voices active` : "Single voice active";
    ...
    <main className="relative h-dvh overflow-hidden bg-[#03040a] text-white">
      ...
      <div className="relative z-10 mx-auto flex h-dvh w-full max-w-4xl flex-col overflow-hidden px-4 pb-4 pt-20 sm:px-6 sm:pt-24">
        ...
        <div className="mt-5 flex-shrink-0">
          <button
            type="button"
            onClick={() => setIsPersonaSelectorOpen((v) => !v)}
            className="inline-flex min-h-11 items-center gap-2 rounded-full border border-white/10 bg-white/[0.04] px-4 py-2 text-xs text-white/60 transition-colors hover:text-white uppercase tracking-[0.16em]"
          >
            <span
              className="h-2 w-2 rounded-full"
              style={{ backgroundColor: `var(${selectedPersona.auroraColor})` }}
            />
            {selectedPersona.name}
            {isRoundtable && selectedPersonas.length > 1 ? ` + ${selectedPersonas.length - 1} more` : ""}
            <span className="ml-1 opacity-50">{isPersonaSelectorOpen ? "▲" : "▼"}</span>
          </button>
        </div>

        {/* Phase 1 fix: Embodiment Tribunal — maxHeight 38vh + overflow-y auto so it never pushes chat off screen */}
        {isPersonaSelectorOpen ? (
          <section
            className="mt-3 flex-shrink-0 rounded-[1.75rem] border border-white/10 bg-white/[0.04] p-4 shadow-[0_18px_60px_rgba(0,0,0,0.18)] backdrop-blur-md"
          ...
          >
            <div className="flex flex-wrap items-center justify-between gap-3">
              <div>
                ...
                <p className="mt-2 text-sm leading-6 text-white/62">{selectorCountLabel}</p>
              </div>
              <button
                type="button"
                onClick={() => {
                  const next = !isRoundtable;
                  setIsRoundtable(next);
                  if (!next) setSelectedPersonaSlugs((s) => [s[0] ?? getRoomPersona("blackboard").slug]);
                }}
                className={`min-h-11 rounded-full border px-4 py-2 text-xs uppercase tracking-[0.16em] transition-colors ${
                  isRoundtable
                    ? "border-gv-aurora-cyan/25 bg-gv-aurora-cyan/10 text-white"
            ...
            <div className="mt-4 flex flex-wrap gap-2">
              {availablePersonas.map((persona) => {
                const active = selectedPersonaSlugs.includes(persona.slug);
                return (
                  <button
                    key={persona.slug}
                    type="button"
                    ...
                    onClick={() => {
                      if (!isRoundtable) {
                        setSelectedPersonaSlugs([persona.slug]);
                        setIsPersonaSelectorOpen(false);
                        return;
                      }
                      setSelectedPersonaSlugs((current) => {
                        if (current.includes(persona.slug)) {
                          const next = current.filter((s) => s !== persona.slug);
                          return next.length > 0 ? next : current;
                        }
                        return [...current, persona.slug];
                      });
                    }}
                    className={`inline-flex min-h-11 items-center gap-2 rounded-full border px-4 py-2 text-sm transition-colors ${
                      active ? "text-white" : "border-white/10 bg-white/[0.03] text-white/62 hover:text-white"
                    }`}
                    style={active ? { borderColor: `var(${persona.auroraColor})`, backgroundColor: "rgba(255,255,255,0.05)" } : undefined}
                  >
                    <span className="h-2.5 w-2.5 rounded-full" style={{ backgroundColor: `var(${persona.auroraColor})` }} />
                    {persona.name}
                  </button>
                );
              })}
        ...
        >
          ...
          {isSending ? (
            <div className="bbr-message-row flex items-end gap-3 justify-start bbr-msg-enter">
              ...
              <div className="bbr-bubble-di flex items-center gap-2 px-4 py-3">
                <span className="bbr-typing-dot" />
                <span className="bbr-typing-dot" />
                <span className="bbr-typing-dot" />
              </div>
            </div>
        ...
        {sessionIsEnded && summaryCaptures.length > 0 ? (
          <section className="mt-4 flex-shrink-0 rounded-[2rem] border border-cyan-300/12 bg-cyan-300/[0.05] p-5 shadow-[0_18px_60px_rgba(0,0,0,0.18)] backdrop-blur-md sm:p-5">
