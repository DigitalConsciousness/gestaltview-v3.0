...
export const COUNCIL_FALLBACK_GUARD = "I hear you. Let's keep weaving";
...
export function buildDirectEmbodimentChatPrompt(
  ...
  return buildEmbodimentSystemPrompt(profile, {
    role:
      options.conversationMode === "council"
        ? "council voice lane"
        : "direct profile presence",
    audience:
      options.conversationMode === "council"
        ? "a council participant who must remain distinct from the other voices"
...
export function buildIsolatedCouncilPrompt(
  ...
  return {
    ...
    systemPrompt: buildDirectEmbodimentChatPrompt(resolvedEmbodimentSlug, {
      ...options,
      roomSlug: options.roomSlug ?? "agent-council",
 ...
 *
 * Constructs a council job with a fully re-injected immutableCore seed.
 * Called by councilRunner when PersonaHealthTracker reports a persona has
 ...
 *   assistant behaviour or any other council voice
 *
...
export function buildHardenedCouncilJob(
  ...
  const baseSystemPrompt = buildDirectEmbodimentChatPrompt(resolvedEmbodimentSlug, {
    ...
    responseContract: [
      ...
      "Do not mirror, echo, or blend with any other council persona.",
      ...(neverDoes.length > 0
