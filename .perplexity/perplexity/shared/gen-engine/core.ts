...
function pickBestTitle(request: Partial<ArtifactSynthesisRequest>): string {
  return (
    normalizeWhitespace(request.sourceTitle ?? "") ||
    normalizeWhitespace(request.title ?? "") ||
    normalizeWhitespace(request.sourceSummary ?? "") ||
    normalizeWhitespace(request.sourceText ?? "") ||
    "Untitled artifact"
  );
}
...
function pickBestSummary(request: Partial<ArtifactSynthesisRequest>): string {
  return (
    normalizeWhitespace(request.summary ?? "") ||
    normalizeWhitespace(request.sourceSummary ?? "") ||
    normalizeWhitespace(request.userInstructions ?? "") ||
...
function renderMarkdownArtifact(input: {
  ...
  const noteBlock = input.userInstructions ? `\n\n## User Instructions\n\n${input.userInstructions}` : "";
...
${normalizeBlock(input.sourceText) || "_No source text provided._"}${noteBlock}`;
}
