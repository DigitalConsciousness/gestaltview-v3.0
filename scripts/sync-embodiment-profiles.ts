...
function normalizeRuntimeProfile(row: {
  ...
  return sortObjectKeys({
    ...
    readinessScore: normalizeReadinessScore(row.readiness_score, 0),
  }) as RawProfile;
}
...
function buildProfileRow(profile: RawProfile, slug: string) {
  ...
  const readinessScore =
    normalizeReadinessScore(profile.readinessScore, founderOnly ? 0.5 : 1);
...
function buildStoredProfileSnapshot(row: {
  ...
  return {
    ...
    readiness_score: normalizeReadinessScore(row.readiness_score, 0),
...
function shouldIncludeInRegistry(profile: RawProfile, includeFounderOnly: boolean): boolean {
  ...
  if (profile.visibilityScope === "internal") {
    return false;
  }
...
async function main() {
  ...
  if (options.dryRun) {
    ...
    const visibilityFilter = options.includeFounderOnly
      ? ["public", "founder-only", "enterprise"]
      : ["public", "enterprise"];
