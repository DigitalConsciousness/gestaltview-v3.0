    ...
    try {
      ...
      setResumeData(prev => {
        ...
        if (aiResult.newExperienceBullet) {
            const currentRole = "Professional Experience"; // This is hardcoded for the demo
...
/**
 * A self-contained demo component for the Resume Rockstar application.
 *
 * IMPORTANT: This component requires Tailwind CSS. Ensure the Tailwind CDN script
 * is included in your platform's main HTML file for proper styling.
...
const ResumeRockstarDemo = () => {
  ...
  return (
    <div className="resume-rockstar-demo relative w-full h-full bg-slate-950 text-slate-100 antialiased overflow-hidden">
      ...
      <button
        onClick={() => navigate('/')}
        className="absolute z-20 left-4 top-4 text-slate-200/80 hover:text-white text-sm flex items-center gap-1 px-3 py-1.5 rounded-full bg-slate-900/70 border border-slate-700/70 backdrop-blur transition-colors"
      >
        ← Home
      </button>

      <div className="neural-aurora-bg">
        <div className="absolute top-1/4 left-1/4 w-96 h-96 bg-teal-500 animate-[pulse_8s_cubic-bezier(0.4,0,0.6,1)_infinite]" />
        <div className="absolute bottom-1/4 right-1/4 w-96 h-96 bg-purple-500 animate-[pulse_8s_cubic-bezier(0.4,0,0.6,1)_2s_infinite]" />
        <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-64 h-64 bg-cyan-400 animate-[pulse_8s_cubic-bezier(0.4,0,0.6,1)_4s_infinite]" />
      </div>
