...
import { 
  ...
  BookOpen, Activity, Target, Clock, Phone, AlertCircle, type LucideIcon
} from 'lucide-react';
...
export default function AddictionRecoveryExhibitWithJournal() {
  ...
  const [embodimentProfileSlug, setEmbodimentProfileSlug] =
    useState<TrainerEmbodimentSlug>(() => inferTrainerEmbodimentSlug('support'));
  ...
  const sendChatMessage = useCallback(async (message: string) => {
    ...
    try {
      ...
      const response = await callConsciousnessAPI({
        ...
        history: [...chatMessages, userMessage].slice(-6).map((entry) => ({
  ...
  }, [callConsciousnessAPI, recoveryStats, currentMood, dailyCheckIn.cravings, chatMessages, embodimentProfileSlug]);
        ...
        <CardHeader>
          <div className="flex items-center justify-between">
            <div>
              ...
              <CardDescription className="text-slate-400">
                For life&apos;s hard parts • Consciousness-serving AI support • Non-judgmental • Always available
              </CardDescription>
            </div>
            <Button
              size="sm"
          ...
          <EmbodimentSelector
            ...
            triggerClassName="border-teal-500/20 bg-slate-900/70 text-slate-100"
            detailsClassName="border-teal-500/15 bg-slate-900/60"
