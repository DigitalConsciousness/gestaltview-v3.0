    ...
    try {
      ...
      const updateAudioLevel = () => {
        if (analyserRef.current && isListening) {
          ...
          const average = dataArray.reduce((acc: number, val: number) => acc + val, 0) / dataArray.length;
  ...
  const startListening = useCallback(() => {
    ...
    try {
      ...
      recognitionRef.current.onresult = (event: any) => {
        ...
        const interimText = interimChunks.join(' ').replace(/\s+/g, ' ').trim();

        if (finalChunks.length > 0) {
          for (const chunk of finalChunks) {
            const normalizedChunk = chunk.trim();
            if (!normalizedChunk || normalizedChunk === lastFinalChunkRef.current) {
              continue;
            }

            lastFinalChunkRef.current = normalizedChunk;
            setTranscript((prev) => (prev ? `${prev} ${normalizedChunk}` : normalizedChunk));

            if (autoSubmit) {
              onTranscriptReceived?.(normalizedChunk);
