...
import { Zap, BrainCircuit, Wind, Lightbulb, Coffee, ArrowLeft, RotateCw, MessageSquare, BookOpen, Send, CheckCircle, ThumbsUp, ThumbsDown, type LucideIcon } from 'lucide-react'
...
type PowerUp = { category: string; title: string; description: string; duration: number; icon: LucideIcon; color: string; };
        ...
        <div className="min-h-screen bg-gradient-to-br from-slate-950 via-slate-900 to-black text-white p-4 sm:p-8">
            <header className="text-center mb-8 max-w-4xl mx-auto">
                ...
                <motion.p 
                    ...
                    ADHD Power-Up is now framed as a scaffolding lane inside the External Scaffold Of You module family.
                    Use Power-Ups for quick resets, chat with your AI companion for clarity, or reflect in your private journal.
                </motion.p>
            </header>
...
const TabButton: React.FC<{id: Tab, activeTab: Tab, setActiveTab: (id: Tab) => void, icon: LucideIcon, children: React.ReactNode}> = ({ id, activeTab, setActiveTab, icon: Icon, children }) => (
    <button
...
const AICompanion = () => {
    ...
    const [embodimentProfileSlug, setEmbodimentProfileSlug] =
        useState<TrainerEmbodimentSlug>(() => inferTrainerEmbodimentSlug('companion'));
    ...
    const handleSendMessage = useCallback(async () => {
        ...
        try {
            const response = await callConsciousnessAPI({
                exhibit: 'adhd-power-up',
                message: currentInput,
                context: {
                    energyLevel,
                    adhdState,
                    cognitiveScaffolding: true,
                },
                embodimentProfileSlug,
                history: [...messages, userMsg].map((message) => ({
            ...
            const aiMsg: ChatMessage = {
                ...
                type: 'ai',
                content: response || "Let's shrink the chaos into one doable next move.",
            };
    ...
    }, [userInput, isLoading, energyLevel, adhdState, embodimentProfileSlug, callConsciousnessAPI, messages]);
        ...
        <Card className="max-w-3xl mx-auto glass-panel border-purple-500/30">
            <CardHeader>
                ...
                <EmbodimentSelector
                    ...
                    triggerClassName="border-slate-700 bg-slate-900/60"
                    detailsClassName="border-slate-700 bg-slate-900/40"
