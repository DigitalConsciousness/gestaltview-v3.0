...
export const TRANSCRIPTORY_BLACKBOARD_HANDOFF_KEY = "gestaltview.transcriptory.blackboardHandoff.v1";
export const TRANSCRIPTORY_CREATION_HANDOFF_KEY = "gestaltview.transcriptory.creationHandoff.v1";

export function buildTranscriptoryHandoffText(capture: TranscriptoryCapture): string {
...
export function formatTranscriptoryFailureMessage(error: unknown, fallback = "Transcriptory capture failed."): string {
...
export function writeTranscriptoryHandoff(destination: "blackboard" | "creation", capture: TranscriptoryCapture): void {
  if (typeof window === "undefined") return;
  const key = destination === "blackboard" ? TRANSCRIPTORY_BLACKBOARD_HANDOFF_KEY : TRANSCRIPTORY_CREATION_HANDOFF_KEY;
...
export async function listTranscriptoryCaptures(filters?: {
  ...
  const response = await fetch(`/api/transcriptory/captures${buildQuery(filters)}`, {
    method: "GET",
    credentials: "include",
  });

  if (!response.ok) {
    throw new Error(response.status === 401 ? "Sign in to load Transcriptory captures." : "Failed to load Transcriptory captures.");
  }
...
export async function getTranscriptoryCapture(captureId: string): Promise<{
  ...
  const response = await fetch(`/api/transcriptory/captures/${encodeURIComponent(captureId)}`, {
    method: "GET",
    credentials: "include",
  });

  if (!response.ok) {
    throw new Error(response.status === 401 ? "Sign in to load Transcriptory capture details." : "Failed to load Transcriptory capture.");
  }
...
export async function deleteTranscriptoryCapture(captureId: string): Promise<void> {
  const response = await fetch(`/api/transcriptory/captures/${encodeURIComponent(captureId)}`, {
    method: "DELETE",
    credentials: "include",
  });
  ...
  if (!response.ok) {
    throw new Error(response.status === 401 ? "Sign in to delete Transcriptory captures." : "Failed to delete Transcriptory capture.");
  }
}
...
export async function createTranscriptoryCapture(input: {
  ...
  if (!response.ok) {
    throw new Error(response.status === 401 ? "Sign in to save Transcriptory captures." : "Failed to save Transcriptory capture.");
  }
...
export async function createTranscriptorySession(input: {
  ...
  if (!response.ok) {
    throw new Error(response.status === 401 ? "Sign in to create Transcriptory sessions." : "Failed to create Transcriptory session.");
  }
...
export async function listTranscriptorySessions(): Promise<TranscriptorySession[]> {
  const response = await fetch("/api/transcriptory/sessions", {
    method: "GET",
    credentials: "include",
  });
  ...
  if (!response.ok) {
    throw new Error(response.status === 401 ? "Sign in to load Transcriptory sessions." : "Failed to load Transcriptory sessions.");
  }
...
export async function requestTranscriptoryHandoff(
  captureId: string,
  target: TranscriptoryHandoffTarget,
): Promise<TranscriptoryHandoffPayload> {
  const response = await fetch(`/api/transcriptory/captures/${encodeURIComponent(captureId)}/handoff`, {
    method: "POST",
    credentials: "include",
    headers: { "Content-Type": "application/json" },
  ...
  if (!response.ok) {
    throw new Error(response.status === 401 ? "Sign in to send Transcriptory handoffs." : "Failed to create Transcriptory handoff.");
  }

  const data = await response.json() as { handoff?: TranscriptoryHandoffPayload };
  if (!data.handoff) {
    throw new Error("Transcriptory handoff response was empty.");
  }
  return data.handoff;
}
...
export function createLocalTranscriptoryCapture(input: {
  ...
  return {
    ...
    summary: input.rawTranscript ? "Transcript text saved locally. Server summary generation is pending." : "",
    themes: [],
    linkedCaptures: [],
