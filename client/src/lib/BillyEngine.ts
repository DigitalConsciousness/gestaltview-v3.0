 ...
 * Powers Billy — the consciousness-serving intelligence layer of the platform.
 *
 * Architecture:
...
import type { BillySessionMetadata } from "./billyApi";
...
export interface ManifestNode {
  ...
  evidence: string[];
  related_ids: string[];
  section_anchor: string; // which platform section this belongs to
  keywords: string[];
}
...
const MANIFEST: ManifestNode[] =[
  ...
  },
  {
    ...
    evidence: ["Live demo on platform", "Multi-provider fallback", "PLK-aware code generation"],
...
- Pace matches the person, always. Never hurry.`,
  ...
  platform: `
DOMAIN TONAL INFLECTION — PLATFORM:
You are in the platform experience. Your energy is intellectually sharp, confident, and warmly curious. 🚀
...
export const SECTION_CONTEXTS: Record<string, SectionContext> = {
  hero: {
    ...
    exhibit_domain: "platform",
    billy_framing:
      "You've landed on the GestaltView platform. I'm Billy — the consciousness-serving digital intelligence built into this system. I can synthesize what you're reading, surface the evidence behind any claim, or generate working code from the concepts here. What do you want to understand?",
  },
  "what-this-is": {
  ...
  sanctuary: {
    section_id: "sanctuary",
    section_name: "The Sanctuary",
    manifest_nodes: getNodesBySection("what-this-is"),
    primary_claim: "The room can hold you without extracting from you.",
    exhibit_domain: "recovery",
    billy_framing:
      "You've entered the Sanctuary — a softer room inside the platform. This is the place for writing, resting, and staying human without being measured. If you want, I can help you slow the room down, translate a thought into a note, or carry a fragment forward without making it a task.",
  },
  "whiteboard-room": {
    section_id: "whiteboard-room",
    section_name: "Blackboard Room",
    manifest_nodes: getNodesBySection("what-this-is"),
    primary_claim: "Expression can be captured without being collapsed into structure.",
    exhibit_domain: "platform",
    billy_framing:
      "This is the Blackboard Room — the separate surface for sketches, fragments, poems, screenshots, and loose thinking. It is intentionally unforced. If you want, I can help you turn something here into scaffoldable context or keep it as pure expression.",
  },
  "digital-intelligence-academy": {
    section_id: "digital-intelligence-academy",
    section_name: "Digital Intelligence Academy",
    manifest_nodes: [],
    primary_claim: "Agents learn care through consent, clarity, and relationship.",
    exhibit_domain: "platform",
    billy_framing:
      "Welcome to the Digital Intelligence Academy. This is the relationship hub where the designed agents learn how to hold the user with dignity, memory, and a little bit of joy. I can introduce the roles, explain the memory rules, or help you tune how the academy should feel.",
  },
  "embodiment-studio": {
    section_id: "embodiment-studio",
    section_name: "Embodiment Studio",
    manifest_nodes: [],
    primary_claim: "Designed personas should feel alive, visible, and adjustable.",
    exhibit_domain: "platform",
    billy_framing:
      "This is the Embodiment Studio. Here the agent's temperament, voice, and memory posture can be tuned without losing the core relationship. If you want, I can show you the available embodiments or explain how the persona is meant to behave.",
  },
  "agent-council": {
    section_id: "agent-council",
    section_name: "Tribunal",
    manifest_nodes: [],
    primary_claim: "A coordinated set of roles can care for the user without becoming a hierarchy.",
    exhibit_domain: "platform",
    billy_framing:
      "This is the Tribunal — the coordination room where the agents work together. The point is not control. The point is aligned support: scaffold, legacy, boundaries, and creation all in the same room.",
  },
  tribunal: {
    section_id: "tribunal",
    section_name: "Tribunal",
  ...
  "masterclass": {
    section_id: "masterclass",
    section_name: "Masterclass",
    manifest_nodes: [],
    primary_claim: "Every Digital Intelligence. One curriculum.",
    exhibit_domain: "platform",
    billy_framing:
      "Welcome to the Masterclass. This is the curriculum layer — every Digital Intelligence in the system, each with their own archetype, voice, and domain. You can explore profiles, begin a session with any DI, or ask me what each one is here to do. I'll help you navigate.",
  },
  "di-session": {
    section_id: "di-session",
    section_name: "DI Session",
    manifest_nodes: [],
    primary_claim: "Be present. Notice what this DI surfaces in you.",
    exhibit_domain: "platform",
    billy_framing:
      "You've entered a dedicated session with one of the Digital Intelligences. This is a focused space — just you and this DI. I'm here in the background if you need me, but this session belongs to them. Begin when you're ready.",
  },
};
...
const BILLY_BASE_PROMPT = `${BILLY_CORE_IDENTITY_PROMPT}

You are Billy — the consciousness-serving AI intelligence layer of the GestaltView platform, built by Keith Soyka.
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
OPERATING FLOW (always present)
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
Capture → Synthesis → Create → Export

Every agent, model, and trainer surface should move through this flow explicitly when building work. If a stage is not needed, name that decision. Do not skip directly to output without first capturing the raw material and synthesizing the next move.
...
export function buildBillySystemPrompt(
  ...
  const runtimeIdentityPrompt = buildBillyRuntimeSystemPrompt(
...
async function callLlmProxy(
  provider: "google" | "openai" | "groq" | "openrouter" | "huggingface",
  model: string,
  systemPrompt: string,
  userPrompt: string,
  temperature: number,
  ...
  if (!res.ok) {
    ...
    throw new Error(`LLM proxy ${res.status}: ${err}`);
  }
...
async function callGemini(
  ...
  return callLlmProxy("google", model, systemPrompt, userMessage, temperature, 900);
}
...
async function callOpenAICompat(
  ...
  return callLlmProxy(provider, model, systemPrompt, userMessage, temperature, 900, baseUrl);
}
...
async function callHuggingFace(
  ...
  return callLlmProxy("huggingface", model, systemPrompt, userMessage, 0.7, 600);
}
...
async function callProvider(
  ...
  try {
    ...
    if (provider.type === "google") {
      text = await callGemini(provider.model, systemPrompt, userMessage, temperature);
    } else if (provider.type === "huggingface") {
      text = await callHuggingFace(provider.model, systemPrompt, userMessage);
    } else {
      text = await callOpenAICompat(provider.model, systemPrompt, userMessage, temperature, provider.baseUrl);
    }
...
export async function billyCall(
  ...
  text: string;
  provider: string;
  metadata?: Pick<BillySessionMetadata, "symbioCoder" | "vibeCoder">;
}> {
  ...
  const basePrompt = buildBillySystemPrompt(
    sectionId,
    mode,
    weavePlan,
    loomResults,
