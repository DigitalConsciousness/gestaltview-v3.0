...
import type { DICapabilityManifest } from "../shared/di/index.js";
import {
  buildSessionThread,
  evaluateForMemory,
...
function parseCapabilities(value: unknown): DICapabilityManifest | undefined {
  if (!value || typeof value !== "object") {
    return undefined;
  }

  const candidate = value as Partial<DICapabilityManifest>;
  const skills = Array.isArray(candidate.skills)
    ? candidate.skills.filter(
        (skill: NonNullable<DICapabilityManifest["skills"]>[number] | undefined) =>
          Boolean(skill?.id && skill?.label && skill?.summary),
      )
    : undefined;
  const tools = Array.isArray(candidate.tools)
    ? candidate.tools.filter(
        (tool: NonNullable<DICapabilityManifest["tools"]>[number] | undefined) =>
          Boolean(tool?.id && tool?.label && tool?.summary),
      )
    : undefined;
  const manifest: DICapabilityManifest = {};

  if (skills && skills.length > 0) {
    manifest.skills = skills as DICapabilityManifest["skills"];
  }
  if (tools && tools.length > 0) {
    manifest.tools = tools as DICapabilityManifest["tools"];
  }
  if (typeof candidate.skillCallPolicy === "string" && candidate.skillCallPolicy.trim()) {
    manifest.skillCallPolicy = candidate.skillCallPolicy.trim();
  }
  if (typeof candidate.toolCallPolicy === "string" && candidate.toolCallPolicy.trim()) {
    manifest.toolCallPolicy = candidate.toolCallPolicy.trim();
  }
