...
//   • SymbioCoder intent/emotion/flow analysis (injected pre-LLM),
//   • VibeCoder vibe alignment scoring (post-LLM, measures voice preservation).
...
import { applyBillyToneGuard } from "../shared/billy/toneGuard.js";
import type { BillyTier, RetrievedChunk, RetrievedMemoryEntry } from "../shared/billy/types.js";
import { analyzeSymbio, type SymbioAnalysis } from "../shared/modules/symbioCoder.js";
import { calculateVibeScore, type VibeResult } from "../shared/modules/vibeCoder.js";
...
export default async function handler(
  ...
  const baseUserMessage = messages.find((message) => message.role === "user")?.content ?? rawMessage;

  // ── SymbioCoder: intent / emotion / flow analysis ──────────────────────────
  // Synchronous, no LLM. Runs on the raw user message.
  // systemPromptFragment is appended to the system prompt.
  // userPromptFragment is prepended to the user message when non-empty.
  // Both steps are non-fatal — failures fall through to the unmodified prompts.
  let symbioAnalysis: SymbioAnalysis | null = null;
  let symbioSystemPrompt = baseSystemPrompt;
  let symbioUserMessage = baseUserMessage;

  try {
    symbioAnalysis = analyzeSymbio(rawMessage);
    if (symbioAnalysis.systemPromptFragment) {
      symbioSystemPrompt = `${baseSystemPrompt}\n\n${symbioAnalysis.systemPromptFragment}`;
    }
    if (symbioAnalysis.userPromptFragment) {
      symbioUserMessage = `${symbioAnalysis.userPromptFragment}\n\n${baseUserMessage}`;
    }
  } catch (symbioError) {
    console.warn("[Billy] SymbioCoder analysis failed (non-fatal):", symbioError);
  }
  // ─────────────────────────────────────────────────────────────────────────
  ...
  const result = await traceBraintrust(
    {
      ...
      metadata: {
        ...
        memoryRetrievalMode,
        embedBackend,
        symbioRoute: symbioAnalysis?.routing ?? null,
        symbioIntent: symbioAnalysis?.intent.primary ?? null,
        symbioEmotion: symbioAnalysis?.emotion.tone ?? null,
      },
    },
    async (span: BraintrustSpan | null) => {
      span?.log({
        input: symbioUserMessage,
        metadata: {
          section,
          packageFilter,
      ...
      return routeLlm(symbioUserMessage, {
        userId,
        mode: conversationMode,
        exhibit: exhibitDomain ?? section,
        plk: plkProfile,
        tier: userTier,
        systemPrompt: symbioSystemPrompt,
      });
    },
  );
  ...
  // ── VibeCoder: voice preservation scoring ─────────────────────────────────
  // Compares rawMessage (source voice) against result.response (Billy's output).
  // Non-fatal — failures surface as null in metadata.
  let vibeResult: VibeResult | null = null;
  try {
    vibeResult = calculateVibeScore(rawMessage, guardedResponse);
  } catch (vibeError) {
    console.warn("[Billy] VibeCoder scoring failed (non-fatal):", vibeError);
  }
  // ─────────────────────────────────────────────────────────────────────────
  ...
  void logSession(userId, rawMessage, guardedResponse, result.provider, conversationMode, {
    retrievalMode,
    contextSources: chunks.length,
    skillSources: skillCount,
    memorySources: memoryEntries.length,
    ...
    founderContinuityState,
    symbioRoute: symbioAnalysis?.routing ?? null,
    vibeAlignment: vibeResult?.alignment ?? null,
    vibeScore: vibeResult?.score ?? null,
  });
  ...
  // Session closeout stays on the memory lane; approval-gated identity writes
  // are handled by the separate identity memory writer helper.
  ...
  sendJson(
    ...
    200,
    {
      ...envelope(guardedResponse, result.provider, {
        free: result.free,
        tokensUsed: result.tokensUsed ?? null,
        processingTime: result.processingTime ?? 0,
        metadata: {
          ...
          embedModel,
          gravity: gravityMetadata,
          symbioCoder: symbioAnalysis
            ? {
                route: symbioAnalysis.routing,
                intent: symbioAnalysis.intent,
                emotion: symbioAnalysis.emotion,
                flow: symbioAnalysis.flow,
                systemPromptFragmentInjected: Boolean(symbioAnalysis.systemPromptFragment),
                userPromptFragmentInjected: Boolean(symbioAnalysis.userPromptFragment),
                analysisMs: symbioAnalysis.analysisMs,
              }
            : null,
          vibeCoder: vibeResult
            ? {
                score: vibeResult.score,
                alignment: vibeResult.alignment,
                conceptPreservation: vibeResult.conceptPreservation,
                metaphorBonus: vibeResult.metaphorBonus,
                recommendations: vibeResult.recommendations,
              }
            : null,
        },
      }),
