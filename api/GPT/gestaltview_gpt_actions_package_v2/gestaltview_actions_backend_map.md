...
## 13. `GET /actions/embodiment_profiles`

**Purpose**
Expose action-safe embodiment profile summaries generated from `shared/embodiment/generated.ts` and governed by `shared/embodiment/index.ts`.

**Implementation note**
Read-only. Returns identity anchors, public presentation, room bindings, heartbeat metadata, readiness/depth, and safe agent metadata. Does not expose credentials or direct database mutation.

## 14. `GET /actions/runtime`

**Purpose**
Expose the deployed ChatGPT import runtime for `https://gestaltview-di-gsvw.vercel.app/api`, including room embodiment defaults and embodied chat surfaces.

## 15. `GET /actions/features`

**Purpose**
Expose an action-facing feature manifest so ChatGPT knows which tool lanes are available.

## 16. `GET /actions/logic`

**Purpose**
Expose routing, embodiment-governance, and response guardrails for Custom GPT behavior.
