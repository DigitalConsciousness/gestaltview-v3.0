...
function parseConfiguredEmails(rawValue: string | undefined): string[] {
  return (rawValue ?? "").split(",").map(normalizeEmail).filter(Boolean);
}
...
export function hasFounderOrAdminAccess(
  user: Pick<AuthUser, "email" | "isAdmin">,
): boolean {
...
const ADMIN_GATED_PATHS = ["/api/trainer/", "/api/admin/", "/api/gate/"];
...
const AUTH_PROFILE_CACHE = new Map<
  string,
  { user: AuthUser; cachedAt: number }
>();
...
export async function getAuthUser(
  ...
  const cookieToken = parseCookieValue(
    readCookieHeader(req as VercelRequest),
    SESSION_COOKIE_NAME,
  );
  if (cookieToken) {
...
export function isAuthConfigured(): boolean {
  return Boolean(
    getSessionSecret() && (process.env.ADMIN_PASSWORD_HASH ?? "").trim(),
  );
}
