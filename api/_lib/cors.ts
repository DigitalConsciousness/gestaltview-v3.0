...
const PRODUCTION_FALLBACK_ORIGIN = "https://gestaltview-digital-intelligence.vercel.app";
...
export function resolveCorsOrigin(req: VercelRequest, defaultOrigin = "*"): string {
  ...
  if (configuredOrigins.length === 0) {
    if (process.env.NODE_ENV === "production" || process.env.VERCEL_ENV === "production") {
      const normalizedDefault = normalizeOrigin(defaultOrigin);
      return normalizedDefault ?? PRODUCTION_FALLBACK_ORIGIN;
    }
