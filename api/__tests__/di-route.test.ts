...
describe("DI route", () => {
  ...
  it("parses capabilities, injects them into the system prompt, and returns capability notes", async () => {
    const res = createRes();
    await diHandler(
      {
        method: "POST",
        ...
        body: {
          diSlug: "billy",
          message: "Help me inspect coverage gaps.",
          mode: "chat",
          capabilities: {
            skills: [
              {
    ...
    const [, llmInput] = routeLlmMock.mock.calls.at(-1) ?? [];
    expect(llmInput.systemPrompt).toContain("CAPABILITY MANIFEST");
    expect(llmInput.systemPrompt).toContain("Schema auditing: Inspect table coverage and drift.");
    expect(llmInput.systemPrompt).toContain("Seed synthesizer: Generate safe fixture bundles.");
    expect(llmInput.systemPrompt).toContain("Prefer explicit schema facts.");
    expect(llmInput.systemPrompt).toContain("Keep writes additive and scoped.");
    expect(llmInput.systemPrompt).not.toContain("Ignore me");
    expect(llmInput.systemPrompt).not.toContain("broken-tool");

    expect(res.body).toMatchObject({
      diSlug: "billy",
      capabilityNotes: ["skills=1", "tools=1"],
    });
  });
});
