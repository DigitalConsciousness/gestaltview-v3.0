...
describe("workspaces API", () => {
  ...
  it("blocks free users from workspace mutations", async () => {
    const module = await loadModule();
    requireAuthMock.mockResolvedValue({
      id: "user-1",
      email: "keith@example.com",
    ...
    expect(createWorkspaceRoomMock).not.toHaveBeenCalled();
  });

  it("creates a workspace room for paid users", async () => {
    const module = await loadModule();
    requireAuthMock.mockResolvedValue({
      id: "user-1",
      email: "keith@example.com",
