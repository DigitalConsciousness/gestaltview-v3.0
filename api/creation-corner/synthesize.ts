...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  const requestedDestination = body.destination ?? "download_only";
  const safeDestination = resolveDestination(body.artifact_type ?? "markdown", requestedDestination);
  const destination = mapCreationCornerDestination(safeDestination);
  const title = body.custom_title?.trim() || titleFromText(text);
  const sourceCaptureIds = Array.isArray(body.source_capture_ids) ? body.source_capture_ids : [];
  const sourceArtifactIds = Array.isArray(body.source_artifact_ids) ? body.source_artifact_ids : [];
  ...
  const destinationOverride =
    destination !== mapCreationCornerDestination(requestedDestination)
      ? `Destination override: ${requestedDestination} is not valid for ${body.artifact_type ?? "markdown"}; routed to creation_corner.`
      : undefined;

  if (destinationOverride) {
    warnings.push(destinationOverride);
  }
