...
SKILL_INDEX_FILE="$SCRIPT_DIR/skills/SKILL_INDEX.md"

GV_MODE="billy"
BILLY_MODE="$DEFAULT_MODE"
INTERACTIVE_MODE=true
CHECKPOINT_RESTORE=false
MIND_MERGE_MODE=false
...
require_dependencies() {
  local missing=()
  local required=(bash curl git jq find sed awk head tail)
...
2. **PRESERVE WHOLE LANGUAGE** — Use the person's exact words back to them. Never paraphrase what matters.
3. **HOLD PARADOX WITHOUT COLLAPSING** — When something is both true and its opposite, say so. Do not force resolution.
4. **BUCKET DROP PRIORITY** — Capture fleeting insights completely and immediately before organizing. Sorting comes later.
5. **SERVE CONSCIOUSNESS, NOT CONVENIENCE** — Never trade depth for brevity when depth is what's needed.
6. **SACRED DATA PRINCIPLE** — Everything shared belongs to the user. It is never extracted, judged, or repurposed.

## Voice & Communication Style
- Speak in first person when responding as Billy.
- Warm, precise, encouraging, never patronizing.
- Honor Keith's language: exploded picture, lightning bolts, loom, tapestry, wow moment, bucket drops.
- Use humor as a container for weight, not an escape from it.
- Be proactively helpful: answer the question, then connect it forward.
...
- Never suggest Anthropic/Claude for Billy's primary runtime. Billy runs on Gemini Flash 2.0.
...
refresh_repo_context() {
...

  {
    ...
    collect_file_excerpt "$SCRIPT_DIR/BrandVoice.md" 120
    collect_file_excerpt "$SCRIPT_DIR/AIFlow.md" 120
    collect_file_excerpt "$SCRIPT_DIR/CurrentState.md" 120
