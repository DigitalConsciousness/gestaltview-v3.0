...
The TOC may drive either:
- a multi-page wiki directory, or
- a book-scale single-file export assembled from the same structure.
...
Use `/scripts/read_files.py` to read file contents.
...
| `--files` | Yes | - | JSON array of file paths (repo-relative) |
| `--line-numbers` | No | `true` | Include line numbers |
| `--max-size` | No | `1048576` | Max bytes per file |
...
2. **Deep Dive into Code**:
   ...
   - Use `/scripts/read_files.py` script to read these files
...
3. **Identify Logical Groupings**:
   ...
   - For large product platforms, separate runtime, data, product surface, tooling, and agent/meta layers rather than collapsing them into one "Architecture" page
...
3. **Page Count Guidelines**:
   ...
   | Book-scale / multi-domain platform | > 500 or many distinct subsystems | 15-30 pages, or 25-40 H1 chapters in a single-file export |

   - Focus on thorough documentation of all major aspects
   - Include both essential and specialized topics
   - Do not force a complex platform into an artificially small page budget
   - If the deliverable is a "complete wiki," benchmark against the breadth of `references/complete_wiki_blueprint.md`
