...
- Reuse .claude-plugin/plugin.json when present, otherwise fall back to
  gemini-extension.json.
- Discover skills from the curated canonical catalog plus repo-local helpers.
...
def load_plugin_metadata() -> dict:
    if CLAUDE_PLUGIN_MANIFEST.exists():
        return load_json(CLAUDE_PLUGIN_MANIFEST)
    if GEMINI_EXTENSION.exists():
        return load_json(GEMINI_EXTENSION)
    return {}


def build_cursor_plugin_manifest() -> dict:
    src = load_plugin_metadata()

    name = src.get("name")
    if not isinstance(name, str) or not name:
        raise ValueError("No plugin metadata source defines a non-empty 'name'")
    validate_plugin_name(name)

    skills = collect_skills()
    if not skills:
        raise ValueError("No skills discovered from manifest.json or skills/ helpers")

    manifest: dict = {"name": name, "skills": ".", "mcpServers": ".mcp.json"}
